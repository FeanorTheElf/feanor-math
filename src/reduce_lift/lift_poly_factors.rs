...
/// `I^e`. `LiftPolyEvalRing` on the other hand is designed to allow computations modulo multiple
...
/// decomposition into maximal ideals `a = m1 ∩ ... ∩ mr`. Of course, this makes the standard
/// approach much more complicated (and when computing factorizations, we often restrict to the case
/// that `I` itself is indeed maximal).
///
...
///    [`PolyLiftFactorsDomain`]). For simplicity, let's focus on the finite field case, i.e. `R =
...
/// [`crate::reduce_lift::lift_poly_eval::LiftPolyEvalRing`].
pub trait PolyLiftFactorsDomain: Domain + DivisibilityRing + FiniteRingSpecializable {
    ...
    /// [`PolyLiftFactorsDomain::reconstruct_ring_el()`]) their reductions modulo `I^e`.
    ///
    /// Note that this is just a heuristic, and if it does not work, most implementations will
...
pub trait IntegerPolyLiftFactorsDomain: PolyLiftFactorsDomain {
    ...
    type LocalFieldAsZnBase<'ring>: ?Sized
        + CanIsoFromTo<Self::LocalFieldBase<'ring>>
        + PolyTFracGCDRing
        + FactorPolyField
        + Field
        + SelfIso
    ...
    fn local_field_as_zn<'a, 'ring>(&self, local_field: &'a Self::LocalField<'ring>)
    -> &'a Self::LocalFieldAsZn<'ring>;
...
where
    R: 'ring + ?Sized + PolyLiftFactorsDomain,
    'ring: 'data,
{
    ring: RingRef<'data, R>,
...
impl<'ring, 'data, 'local, R> Homomorphism<R, R::LocalRingBase<'ring>>
    for PolyLiftFactorsDomainReductionMap<'ring, 'data, 'local, R>
where
    R: 'ring + ?Sized + PolyLiftFactorsDomain,
    'ring: 'data,
{
...
impl<'ring, 'data, 'local, R> Homomorphism<R::LocalRingBase<'ring>, R::LocalRingBase<'ring>>
    for PolyLiftFactorsDomainIntermediateReductionMap<'ring, 'data, 'local, R>
where
    R: 'ring + ?Sized + PolyLiftFactorsDomain,
    'ring: 'data,
{
...
impl<'ring, 'data, 'local, R> Homomorphism<R::LocalRingBase<'ring>, R::LocalFieldBase<'ring>>
    for PolyLiftFactorsDomainBaseRingToFieldIso<'ring, 'data, 'local, R>
where
    R: 'ring + ?Sized + PolyLiftFactorsDomain,
    'ring: 'data,
{
    ...
    'ring: 'data,
{
    ...
    pub fn new(ring: &'data R, ideal: &'data R::SuitableIdeal<'ring>, e: usize) -> Self {
        ...
        Self {
            ...
            from: (0..maximal_ideal_factor_count)
                .map(|idx| ring.quotient_ring_at(ideal, e, idx))
                .collect::<Vec<_>>(),
            to: (0..maximal_ideal_factor_count)
                .map(|idx| ring.quotient_ring_at(ideal, 1, idx))
                .collect::<Vec<_>>(),
            to_fields: (0..maximal_ideal_factor_count)
                .map(|idx| ring.quotient_field_at(ideal, idx))
                .collect::<Vec<_>>(),
        }
    }
    ...
    pub fn main_ring_to_field_reduction<'local>(
        &'local self,
        max_ideal_idx: usize,
    ) -> PolyLiftFactorsDomainReductionMap<'ring, 'data, 'local, R> {
    ...
    pub fn intermediate_ring_to_field_reduction<'local>(
        &'local self,
        max_ideal_idx: usize,
    ) -> PolyLiftFactorsDomainIntermediateReductionMap<'ring, 'data, 'local, R> {
    ...
    pub fn base_ring_to_field_iso<'local>(
        &'local self,
        max_ideal_idx: usize,
    ) -> PolyLiftFactorsDomainBaseRingToFieldIso<'ring, 'data, 'local, R> {
        PolyLiftFactorsDomainBaseRingToFieldIso {
            ring: self.ring,
            ideal: self.ideal,
...
/// fields, since it would conflict with the one for all rings that impl [`PolyLiftFactorsDomain`].
/// Thus, we implement [`PolyLiftFactorsDomain`] for all finite fields, and reuse the blanket impl.
///
...
impl<'a, R> Domain for IntegersWithZnQuotient<'a, R> where
    R: Sized + SelfIso + ZnRing + FromModulusCreateableZnRing + Clone
{
}

impl<'a, R> DelegateRingImplFiniteRing for IntegersWithZnQuotient<'a, R> where
    R: Sized + SelfIso + ZnRing + FromModulusCreateableZnRing + Clone
{
}

impl<'a, R> DelegateRingImplEuclideanRing for IntegersWithZnQuotient<'a, R> where
    R: Sized + SelfIso + ZnRing + FromModulusCreateableZnRing + Clone
{
}
...
{
    type LocalRingBase<'ring>
        = <<Self as DelegateRing>::Base as crate::reduce_lift::lift_poly_eval::LiftPolyEvalRing>::LocalRingBase<'ring>
    where
        Self: 'ring;
    type LocalRing<'ring>
        = <<Self as DelegateRing>::Base as crate::reduce_lift::lift_poly_eval::LiftPolyEvalRing>::LocalRing<'ring>
    where
        Self: 'ring;
    type LocalComputationData<'ring>
        = <<Self as DelegateRing>::Base as crate::reduce_lift::lift_poly_eval::LiftPolyEvalRing>::LocalComputationData<
        'ring,
    >
    where
        Self: 'ring;
    ...
    R: Sized + SelfIso + ZnRing + FromModulusCreateableZnRing + LinSolveRing + Clone,
{
        ...
        Self: 'el + 'ring,
    {
        let log2_max_coeff = coefficients
            ...
            .max()
            .unwrap_or(0);
        // this is in no way a rigorous bound, but equals the worst-case bound at least asymptotically (up
        // to constants)
        return ((log2_max_coeff as f64 + poly_deg as f64) / self.integers.abs_log2_floor(ideal).unwrap_or(1) as f64
            * crate::reduce_lift::lift_poly_factors::INTRING_HEURISTIC_FACTOR_SIZE_OVER_POLY_SIZE_FACTOR)
            .ceil() as usize
            + 1;
    }
