...
///    values `|ai|` are given by [`EvalPolyLocallyRing::pseudo_norm()`].
...
/// [`crate::reduce_lift::poly_factor_gcd::PolyGCDLocallyDomain`].
///
...
where
    R: ?Sized + FiniteRing + Field + Debug + SelfIso,
{
    type LocalComputationData<'ring>
        = RingRef<'ring, Self>
    where
    ...
    'ring: 'data,
{
    ...
    pub fn new(ring: &'data R, data: &'data R::LocalComputationData<'ring>, index: usize) -> Self {
        Self {
            ring: RingRef::new(ring),
            data,
            local_ring: ring.local_ring_at(data, index),
            index,
        }
    }
}
...
/// [`crate::rings::zn::zn_64::Zn`]. This should be suitable in almost all scenarios.
///
