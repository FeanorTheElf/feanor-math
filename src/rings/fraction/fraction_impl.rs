    ...
    R::Type: Domain,
{
    ...
    fn add_assign_ref(&self, lhs: &mut Self::Element, rhs: &Self::Element) {
        ...
        self.base_ring
            .add_assign(&mut lhs.num, self.base_ring.mul_ref(&lhs.den, &rhs.num));
    ...
    fn add_assign(&self, lhs: &mut Self::Element, rhs: Self::Element) {
        self.base_ring.mul_assign_ref(&mut lhs.num, &rhs.den);
        self.base_ring
            .add_assign(&mut lhs.num, self.base_ring.mul_ref_fst(&lhs.den, rhs.num));
    ...
    fn dbg_within<'a>(
        ...
        if let Some(quo) = self.base_ring.checked_div(&value.num, &value.den) {
            ...
            self.base_ring
                .get_ring()
                .dbg_within(&value.num, out, EnvBindingStrength::Product)?;
            write!(out, "/")?;
            self.base_ring
                .get_ring()
                .dbg_within(&value.den, out, EnvBindingStrength::Product)?;
            if env >= EnvBindingStrength::Product {
                write!(out, ")")?;
            }
            Ok(())
        ...
        Self: 'a,
    {
        ...
        let mut it = elements.map(|x| {
            let gcd = self
                .base_ring
                .get_ring()
                .balance_factor([&denominator_lcm, &x.den].into_iter());
    ...
    R::Type: Domain + IntegerRing + HashableElRing,
{
    fn hash<H: std::hash::Hasher>(&self, el: &Self::Element, h: &mut H) {
        let gcd = signed_gcd(
            self.base_ring.clone_el(&el.den),
            self.base_ring.clone_el(&el.num),
            &self.base_ring,
        );
        self.base_ring
            .get_ring()
            .hash(&self.base_ring.checked_div(&el.num, &gcd).unwrap(), h);
        self.base_ring
            .get_ring()
            .hash(&self.base_ring.checked_div(&el.den, &gcd).unwrap(), h);
    }
}
    ...
    S::Type: Domain,
{
    ...
    fn map_in(
        &self,
        from: &FractionFieldImplBase<S>,
        el: <FractionFieldImplBase<S> as RingBase>::Element,
        hom: &Self::Homomorphism,
    ) -> Self::Element {
        self.from_fraction(
            self.base_ring()
                .get_ring()
                .map_in(from.base_ring().get_ring(), el.num, hom),
            self.base_ring()
                .get_ring()
                .map_in(from.base_ring().get_ring(), el.den, hom),
        )
    }
}
    ...
    S::Type: Domain,
{
    ...
    ) -> <FractionFieldImplBase<S> as RingBase>::Element {
        from.from_fraction(
            self.base_ring()
                .get_ring()
                .map_out(from.base_ring().get_ring(), el.num, iso),
            self.base_ring()
                .get_ring()
                .map_out(from.base_ring().get_ring(), el.den, iso),
        )
    }
}
    ...
    I::Type: IntegerRing,
{
    ...
    fn map_in(
        ...
        self.from_fraction(
            self.base_ring()
                .get_ring()
                .map_in(from.base_ring().get_ring(), num, hom),
            self.base_ring()
                .get_ring()
                .map_in(from.base_ring().get_ring(), den, hom),
        )
    }
}
...
fn test_balance_factor() {
    ...
    assert_el_eq!(
        &ring,
        ring.from_fraction(11, 7 * 6),
        ring.get_ring().balance_factor(elements.iter()).unwrap()
    );
}
...
fn test_ring_axioms() {
    let ring = FractionFieldImpl::new(StaticRing::<i64>::RING);
    let edge_case_elements = multi_cartesian_product(
        [&[-3, -2, -1, 0, 1, 2, 3][..], &[1, 2, 3][..]]
            .into_iter()
            .map(|list| list.iter().copied()),
        |data| ring.from_fraction(data[0], data[1]),
        |_, x| *x,
    );
