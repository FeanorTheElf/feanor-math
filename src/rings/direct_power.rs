...
impl<R: RingStore, const N: usize> RingExtension for DirectPowerRingBase<R, N> {
    ...
    fn mul_assign_base_through_hom<S: ?Sized + RingBase, H: Homomorphism<S, R::Type>>(
        &self,
    ...
    R::Type: CanHomFrom<S::Type>,
{
    ...
    fn map_in(
        ...
        from_fn(|_| {
            self.base
                .get_ring()
                .map_in(from.base.get_ring(), el_it.next().unwrap(), hom)
        })
    }
    ...
    R::Type: CanIsoFromTo<S::Type>,
{
    ...
    fn map_out(
        ...
        from_fn(|_| {
            self.base
                .get_ring()
                .map_out(from.base.get_ring(), el_it.next().unwrap(), iso)
        })
    }
}
    ...
    R::Type: DivisibilityRing,
{
    ...
    fn checked_left_div_prepared(
        ...
        try_from_fn(|i| {
            self.base
                .get_ring()
                .checked_left_div_prepared(&lhs[i], &rhs[i], &rhs_prep[i])
        })
    }
    ...
    fn divides_left_prepared(
        ...
        (0..N).all(|i| {
            self.base
                .get_ring()
                .divides_left_prepared(&lhs[i], &rhs[i], &rhs_prep[i])
        })
    }
}
    ...
    R::Type: FiniteRingSpecializable,
{
    fn specialize<O: FiniteRingOperation<Self>>(op: O) -> O::Output {
        ...
        impl<R: RingStore, O: FiniteRingOperation<DirectPowerRingBase<R, N>>, const N: usize>
            FiniteRingOperation<R::Type> for BaseRingCase<R, O, N>
        {
    ...
    R::Type: FiniteRing,
{
    type ElementsIter<'a>
        = MultiProduct<
        <R::Type as FiniteRing>::ElementsIter<'a>,
        DirectPowerRingElCreator<'a, R, N>,
        CloneRingEl<&'a R>,
        [El<R>; N],
    >
    where
    ...
    fn elements<'a>(&'a self) -> Self::ElementsIter<'a> {
        multi_cartesian_product(
            (0..N).map(|_| self.base.elements()),
            DirectPowerRingElCreator { ring: self },
            CloneRingEl(&self.base),
        )
    }
...
fn test_can_hom_axioms() {
    ...
    crate::ring::generic_tests::test_hom_axioms(
        &from,
        &ring,
        multi_cartesian_product((0..3).map(|_| -4..5), clone_array::<_, 3>, |_, x| *x),
    );
}
