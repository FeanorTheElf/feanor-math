...
use crate::algorithms::int_factor::{factor, is_prime_power};
...
fn find_small_irreducible_poly_base<P, C>(
    poly_ring: P,
    degree: usize,
    convolution: C,
...
where
    ...
    <<P::Type as RingExtension>::BaseRing as RingStore>::Type:
        ZnRing + Field + SelfIso + CanHomFrom<StaticRingBase<i64>>,
    C: ConvolutionAlgorithm<<<P::Type as RingExtension>::BaseRing as RingStore>::Type>,
{
    ...
    if degree > 3 {
        ...
        // next, check for cases where we can take `small_poly(X^high_power)`; these cases are
        // characterized by the fact that we have `degree = small_d * large_d` with `large_d
        // | #F_(q^small_d)*`
        ...
        ZnRing + Field + SelfIso + CanHomFrom<StaticRingBase<i64>>,
{
    ...
    let log2_modulus = poly_ring
        .base_ring()
        .integer_ring()
        .abs_log2_ceil(poly_ring.base_ring().modulus())
        .unwrap();
    let fft_convolution = FFTConvolution::new_with_alloc(Global);
    if fft_convolution.has_sufficient_precision(
        StaticRing::<i64>::RING
        ...
        find_small_irreducible_poly_base(&poly_ring, degree, STANDARD_CONVOLUTION, rng)
    }
}
...
/// based on [`Zn`].
pub type DefaultGaloisFieldImpl =
    AsField<FreeAlgebraImpl<AsField<Zn>, SparseMapVector<AsField<Zn>>, Global, KaratsubaAlgorithm>>;
...
/// uses [`FreeAlgebraImpl`] to compute ring arithmetic.
pub type GaloisFieldBaseOver<R, A = Global, C = KaratsubaAlgorithm> =
    GaloisFieldBase<AsField<FreeAlgebraImpl<R, SparseMapVector<R>, A, C>>>;

impl GaloisField {
    /// Creates a new instance of the finite/galois field `GF(p^degree)`.
    ///
    /// If you need more options for configuration, consider using
    /// [`GaloisField::new_with_convolution()`] or the most general [`GaloisField::create()`].
    ///
    /// # Example
    /// ```rust
    /// # use feanor_math::ring::*;
    ...
    A: Allocator + Clone,
{
    ...
    /// whose base ring is exactly `R`; this prevents infinite generic argument recursion, which
    /// might otherwise happen if each recursive call adds a reference `&` to `R`.
...
fn test_can_hom_from() {
        ...
        C: ConvolutionAlgorithm<R::Type>,
    {
        assert_impl_CanHomFrom::<
            GaloisFieldBase<AsField<FreeAlgebraImpl<R, V, A, C>>>,
