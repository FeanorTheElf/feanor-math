...
/// [`FreeAlgebraImplBase`].
pub type FreeAlgebraImpl<R, V, A = Global, C = KaratsubaAlgorithm> =
    RingValue<FreeAlgebraImplBase<R, V, A, C>>;
...
{
    #[stability::unstable(feature = "enable")]
    pub fn set_convolution<C_new: ConvolutionAlgorithm<R::Type>>(
        self,
            ...
            fn execute<'b, V: 'b + VectorViewSparse<El<R>>>(self, vector: V) -> () {
                for i in (self.rank..(2 * self.rank)).rev() {
                    for (j, c) in vector.nontrivial_entries() {
                        ...
                        self.base_ring
                            .add_assign(&mut self.data[i - self.rank + j], add);
                    }
                }
            }
        }
        ...
        if was_sparse.is_err() {
            for i in (self.rank()..(2 * self.rank())).rev() {
                for j in 0..self.x_pow_rank.len() {
                    ...
                    self.base_ring
                        .add_assign(&mut data[i - self.rank() + j], add);
                }
            }
        }
    }
        ...
        self.base_ring().get_ring() == other.base_ring().get_ring()
            ...
            && (0..self.rank()).all(|i| {
                (i >= self.x_pow_rank.len() && i >= other.x_pow_rank.len())
                    || (i >= self.x_pow_rank.len()
                        && self.base_ring().is_zero(other.x_pow_rank.at(i)))
                    || (i >= other.x_pow_rank.len()
                        && self.base_ring().is_zero(self.x_pow_rank.at(i)))
                    || self
                        .base_ring()
                        .eq_el(self.x_pow_rank.at(i), other.x_pow_rank.at(i))
            })
    }
}
    ...
    C: ConvolutionAlgorithm<R::Type>,
{
    ...
    fn clone_el(&self, val: &Self::Element) -> Self::Element {
        ...
        let mut result_values =
            Vec::with_capacity_in(val.values.len(), self.element_allocator.clone());
    ...
    fn add_assign_ref(&self, lhs: &mut Self::Element, rhs: &Self::Element) {
        ...
        for i in 0..self.rank() {
            self.base_ring()
                .add_assign_ref(&mut lhs.values[i], &rhs.values[i]);
        }
    }
    ...
    fn sub_assign_ref(&self, lhs: &mut Self::Element, rhs: &Self::Element) {
        ...
        for i in 0..self.rank() {
            self.base_ring()
                .sub_assign_ref(&mut lhs.values[i], &rhs.values[i]);
        }
    }
    ...
    fn mul_assign_int(&self, lhs: &mut Self::Element, rhs: i32) {
        ...
        for i in 0..self.rank() {
            self.base_ring()
                .int_hom()
                .mul_assign_map(&mut lhs.values[i], rhs);
        }
    }
    ...
    fn square(&self, value: &mut Self::Element) {
        let mut tmp =
            Vec::with_capacity_in(2 << self.log2_padded_len, self.element_allocator.clone());
        tmp.resize_with(2 << self.log2_padded_len, || self.base_ring().zero());
    ...
    C: ConvolutionAlgorithm<R::Type>,
{
    ...
    fn from(&self, x: El<Self::BaseRing>) -> Self::Element {
        ...
        result_values
            .extend((0..((1 << self.log2_padded_len) - 1)).map(|_| self.base_ring().zero()));
        return FreeAlgebraImplEl {
            values: result_values.into_boxed_slice(),
        };
    }
    ...
    C: ConvolutionAlgorithm<R::Type>,
{
    fn checked_left_div(&self, lhs: &Self::Element, rhs: &Self::Element) -> Option<Self::Element> {
        ...
        if has_sol.is_solved() {
            return Some(self.from_canonical_basis(
                (0..self.rank()).map(|i| self.base_ring().clone_el(solution.at(i, 0))),
            ));
        } else {
            return None;
        }
...
impl<'de, R> Deserialize<'de>
    for FreeAlgebraImplBase<R, SparseMapVector<R>, Global, KaratsubaAlgorithm>
where
    R: RingStore + Deserialize<'de> + Clone,
    R::Type: SerializableElementRing,
{
        ...
        D: Deserializer<'de>,
    {
        ...
        for (c, i) in poly_ring.terms(&poly) {
            *x_pow_rank.at_mut(i) = poly_ring
                .base_ring()
                .negate(poly_ring.base_ring().clone_el(c));
        }
        _ = x_pow_rank.at_mut(0);
        return Ok(FreeAlgebraImpl::new_with_convolution(
        ...
        let x_pow_rank = (0..rank)
            .map(|i| {
                poly_ring.base_ring().negate(
                    poly_ring
                        .base_ring()
                        .clone_el(poly_ring.coefficient_at(&poly, i)),
                )
            })
            .collect::<Vec<_>>();
        return Ok(FreeAlgebraImpl::new_with_convolution(
    ...
    C: ConvolutionAlgorithm<R::Type>,
{
        ...
        W::IntoIter: DoubleEndedIterator,
    {
        ...
        result.extend(
            (0..((1 << self.log2_padded_len) - self.rank())).map(|_| self.base_ring().zero()),
        );
    ...
    fn mul_assign_gen_power(&self, el: &mut Self::Element, power: usize) {
        ...
        gen_pow_d.extend(
            self.x_pow_rank
                .as_iter()
                .map(|c| self.base_ring.clone_el(c)),
        );
    ...
    C: ConvolutionAlgorithm<R::Type>,
{
    extern "rust-call" fn call_mut(&mut self, args: (&'b [El<R>],)) -> Self::Output {
        self.base_ring.from_canonical_basis(
            args.0
                .iter()
                .map(|x| self.base_ring.base_ring().clone_el(x)),
        )
    }
}
    ...
    C: ConvolutionAlgorithm<R::Type>,
{
    type ElementsIter<'a>
        ...
        WRTCanonicalBasisElementCreator<'a, R, V, A, C>,
        CloneRingEl<&'a R>,
        Self::Element,
    >
    where
    ...
    fn elements<'a>(&'a self) -> Self::ElementsIter<'a> {
        multi_cartesian_product(
            ...
            WRTCanonicalBasisElementCreator { base_ring: self },
        ...
        I::Type: IntegerRing,
    {
        ...
        if ZZ.get_ring().representable_bits().is_none()
            || ZZ.abs_log2_ceil(&base_ring_size).unwrap() * self.rank()
    ...
    fn random_element<G: FnMut() -> u64>(&self, mut rng: G) -> <Self as RingBase>::Element {
        self.from_canonical_basis(
            (0..self.rank()).map(|_| self.base_ring().random_element(&mut rng)),
        )
    }
}
...
impl<R1, V1, A1, C1, R2, V2, A2, C2> CanHomFrom<FreeAlgebraImplBase<R1, V1, A1, C1>>
    for FreeAlgebraImplBase<R2, V2, A2, C2>
where
    R1: RingStore,
    V1: VectorView<El<R1>>,
    A1: Allocator + Clone,
        ...
        if self.rank() == from.rank() {
            ...
            if (0..self.rank()).all(|i| {
                (i >= self.x_pow_rank.len() && i >= from.x_pow_rank.len())
                    || (i >= self.x_pow_rank.len()
                        && from.base_ring().is_zero(from.x_pow_rank.at(i)))
                    || (i >= from.x_pow_rank.len()
                        && self.base_ring().is_zero(self.x_pow_rank.at(i)))
...
impl<R1, V1, A1, C1, R2, V2, A2, C2> CanIsoFromTo<FreeAlgebraImplBase<R1, V1, A1, C1>>
    for FreeAlgebraImplBase<R2, V2, A2, C2>
where
    R1: RingStore,
    V1: VectorView<El<R1>>,
    ...
    R2::Type: CanIsoFromTo<R1::Type>,
{
    ...
    fn map_out(
        ...
        from.from_canonical_basis((0..self.rank()).map(|i| {
            self.base_ring.get_ring().map_out(
                from.base_ring.get_ring(),
                self.base_ring.clone_el(&el.values[i]),
                iso,
            )
        }))
    }
...
fn test_ring_axioms() {
    ...
    let ring = ring
        .into()
        .set_convolution(FFTConvolution::new_with_alloc(Global));
    crate::ring::generic_tests::test_ring_axioms(ring, els.into_iter());
    ...
    let els = (0..10).map(|_| {
        ring.from_canonical_basis(
            (0..64).map(|_| ring.base_ring().random_element(|| rng.rand_u64())),
        )
    });
...
fn test_division() {
    ...
    assert_el_eq!(
        ...
        &ring
            .checked_div(
                &ring.one(),
                &ring.from_canonical_basis([i.map(2), i.map(3)])
            )
            .unwrap()
    );
...
fn test_division_ring_of_integers() {
    ...
    assert!(
        ring.checked_div(
            &ring.from_canonical_basis([2, 0]),
            &ring.from_canonical_basis([3, 0])
        )
        .is_none()
    );
}
...
fn test_cubic_mul() {
    ...
    let ring = FreeAlgebraImpl::new(
        base_ring,
        3,
        [modulo.map(-1), modulo.map(-1), modulo.map(-1)],
    );
