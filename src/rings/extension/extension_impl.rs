...
pub struct FreeAlgebraImplBase<
    R,
    V = SparseMapVector<R>,
    C = DynConvolution<'static, <R as RingStore>::Type>,
    A = Global,
> where
    R: RingStore,
...
/// [`FreeAlgebraImplBase`].
pub type FreeAlgebraImpl<R, V = SparseMapVector<R>, C = DynConvolution<'static, <R as RingStore>::Type>, A = Global> =
...
impl<'conv, R, V> FreeAlgebraImplBase<R, V, DynConvolution<'conv, R::Type>, Global>
where
    R: RingStore + 'conv,
    V: VectorView<El<R>> + Send + Sync,
{
    ...
    C: ConvolutionAlgorithm<R::Type>,
{
    ...
    pub fn with_convolution<C_new: ConvolutionAlgorithm<R::Type>>(
        self,
...
where
    R: RingStore + Copy,
    V: VectorView<El<R>> + Send + Sync + Copy + CloneWithRing<R::Type>,
...
impl<'de, 'conv, R> Deserialize<'de>
    for FreeAlgebraImplBase<R, SparseMapVector<R>, DynConvolution<'conv, R::Type>, Global>
where
        ...
        D: Deserializer<'de>,
    {
        ...
        let x_pow_rank = (0..rank)
            .map(|i| {
                poly_ring
                    .base_ring()
                    .negate(poly_ring.base_ring().clone_el(poly_ring.coefficient_at(&poly, i)))
            })
            .collect::<Vec<_>>();
        let base_ring = poly_ring.into().into_base_ring();
        let log2_padded_len = StaticRing::<i64>::RING
    ...
    C: ConvolutionAlgorithm<R::Type>,
{
    type VectorRepresentation<'a>
        = CloneElFn<&'a [El<R>], El<R>, CloneRingEl<&'a R>>
    where
        Self: 'a;
    ...
    C: ConvolutionAlgorithm<R::Type>,
{
    fn specialize<O: FiniteRingOperation<Self>>(op: O) -> O::Output {
        ...
        <R::Type as FiniteRingSpecializable>::specialize(OpWrapper {
            operation: op,
            ring: PhantomData,
        })
    }
    ...
    R2::Type: CanIsoFromTo<R1::Type>,
{
                ...
                (i >= self.x_pow_rank.len() && i >= from.x_pow_rank.len())
                    ...
                    || from.base_ring.eq_el(
                        &self.base_ring.get_ring().map_out(
                            ...
                            self.base_ring.clone_el(self.x_pow_rank.at(i)),
                            &iso,
                        ),
    ...
    fn map_out(
        ...
        from.from_canonical_basis((0..self.rank()).map(|i| {
            self.base_ring
                .get_ring()
                .map_out(from.base_ring.get_ring(), self.base_ring.clone_el(&el.values[i]), iso)
        }))
    }
}
