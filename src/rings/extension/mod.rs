...
///
/// New implementation of Galois fields, to replace the one in [`galois_field`].
/// 
pub mod galois_field_new;
...
/// that is algebraic resp. integral over `S` (i.e. is a zero of a monic polynomial over the
/// base ring). While sounding quite technical, this includes
...
/// 
/// The main implementations are [`extension_impl::FreeAlgebraImpl`] (this is the old implementation)
/// and [`impl_new::FreeAlgebraImpl`] (a new, faster but currently unstable implementation).
/// 
...
/// let galois_field = impl_new::FreeAlgebraImpl::new(prime_field, 3, [2, 1]);
...
/// let galois_field_2 = galois_field_new::GaloisField::new_with(prime_field, 3, Global, STANDARD_CONVOLUTION);
