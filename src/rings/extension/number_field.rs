    ...
    I::Type: IntegerRing,
{
    ...
    {
        let self_ = NumberFieldByOrder {
            base: RingRef::new(poly_ring.base_ring().get_ring()),
        };
...
fn heuristic_factor_poly_directly_in_order<'a, P, Impl, I, Controller>(
    poly_ring: P,
    poly: &El<P>,
    controller: Controller,
) -> HeuristicFactorPolyInOrderResult<P>
where
    Impl: 'a + RingStore,
    Impl::Type: Field + FreeAlgebra,
    <Impl::Type as RingExtension>::BaseRing: RingStore<Type = RationalFieldBase<I>>,
    ...
    Controller: ComputationController,
{
    controller.run_computation(
        format_args!(
            "factor_direct(deg={}, extdeg={})",
            poly_ring.degree(poly).unwrap(),
            poly_ring.base_ring().rank()
        ),
        |controller| {
            let mut rng = oorandom::Rand64::new(1);
            let self_ = poly_ring.base_ring();

            // first, we try to find an inert prime `p` and lift a factorization modulo `p` to the
            // ring
    ...
    I::Type: IntegerRing,
{
        ...
        Self: 'ring,
    {
        ...
        return to.from_terms(
            from.terms(poly)
                .map(|(c, i)| (self.base.inclusion().mul_ref_map(c, &denominator), i)),
        );
    }
    ...
    I: 'ring,
{
    fn lifted_factorization<'a>(
        ...
        DensePolyRing<<I::Type as PolyGCDLocallyDomain>::LocalRing<'ring>>,
        Vec<El<DensePolyRing<<I::Type as PolyGCDLocallyDomain>::LocalRing<'ring>>>>,
    ) {
        let ZZX = &self.ZZX;
        let ZZ = ZZX.base_ring();
    ...
    I::Type: IntegerRing,
{
    type LocalRingBase<'ring>
        = FreeAlgebraImplBase<LocalRing<'ring, I>, SparseMapVector<LocalRing<'ring, I>>>
    where
        Self: 'ring;
    ...
    type LocalFieldBase<'ring>
        = NumberFieldOrderQuotient<'ring, I>
    where
        Self: 'ring;
        ...
        Self: 'ring,
    {
        ...
        let QQ = self.base.base_ring();
        let ZZ = QQ.base_ring();
        // to give any mathematically justifiable value, we would probably have to consider the
        // canonical norm; I don't want to deal with this here, so let's just use the
        // coefficient norm instead...
        ...
        for _ in 0..MAX_PROBABILISTIC_REPETITIONS {
            ...
            if factorization.iter().all(|(_, e)| *e == 1) {
                return NumberRingIdeal {
                    ...
                    Fp_as_zn: ZZ
                        .get_ring()
                        .local_ring_into_zn(Fp_as_ring.clone())
                        .as_field()
                        .ok()
                        .unwrap(),
                    Fp_as_ring,
        ...
        Self: 'ring,
    {
        ...
        let Fp_to_Fp = WrapHom::to_delegate_ring(ideal.Fp_as_zn.get_ring())
            .compose(
                RingRef::new(ideal.Fp_as_zn.get_ring().get_delegate())
                    .into_can_hom(&ideal.Fp_as_ring)
                    .ok()
                    .unwrap(),
            )
            .compose(
                PolyGCDLocallyBaseRingToFieldIso::new(
                    ZZ.get_ring(),
                    &ideal.prime,
                    ideal.Fp_as_ring.get_ring(),
                    FpX.base_ring().get_ring(),
                    0,
                )
                .inv(),
            );
    ...
    {
        assert!(idx < self.maximal_ideal_factor_count(ideal));
        let hom = WrapHom::to_delegate_ring(to.base_ring().get_ring()).compose(
            RingRef::new(to.base_ring().get_ring().get_delegate())
                .into_can_hom(from.base_ring())
                .ok()
                .unwrap(),
        );
        ...
        Self: 'ring,
    {
        assert!(idx < self.maximal_ideal_factor_count(ideal));
        assert!(idx < self.maximal_ideal_factor_count(ideal));
        let hom = RingRef::new(from.base_ring().get_ring().get_delegate())
            .into_can_iso(to.base_ring())
            .ok()
            .unwrap()
            .compose(UnwrapHom::from_delegate_ring(from.base_ring().get_ring()));
        to.from_canonical_basis(from.wrt_canonical_basis(&x).iter().map(|c| hom.map(c)))
    }
