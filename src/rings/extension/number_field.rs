...
where
    ...
    BaseRing<Impl>: RingStore<Ring = RationalFieldBase<I>>,
    I: RingStore,
    I::Ring: IntegerRing,
{
    base: Impl,
...
where
    K: RingStore<Ring = NumberFieldBase<Impl, I>>,
    Impl: RingStore,
    Impl::Ring: Field + FreeAlgebra,
    BaseRing<Impl>: RingStore<Ring = RationalFieldBase<I>>,
    I: RingStore,
...
impl NumberField {
    ...
    where
        P: RingStore,
        P::Ring: PolyRing,
        BaseRing<P>: RingStore<Ring = BigIntRingBase>,
    {
    ...
    where
        P: RingStore,
        P::Ring: PolyRing,
        BaseRing<P>: RingStore<Ring = RationalFieldBase<BigIntRing>>,
    {
    ...
    I::Ring: IntegerRing,
{
    ...
    #[stability::unstable(feature = "enable")]
    pub fn into_choose_complex_embedding<S: RingStore<Ring = Self>>(self_: S) -> ComplexEmbedding<S, Impl, I> {
    ...
    I::Ring: IntegerRing,
{
    ...
    where
        P: RingStore,
        P::Ring: PolyRing,
        BaseRing<P>: RingStore<Ring = I::Ring>,
    {
    ...
    fn by_order(&self) -> &RingValue<NumberFieldByOrder<Impl, I>> {
        unsafe { transmute::<&NumberFieldBase<_, _>, &RingValue<NumberFieldByOrder<_, _>>>(self) }
    }
}
    ...
    I::Ring: IntegerRing,
{
        ...
        BaseRing<P>: RingStore<Ring = Self>,
    {
        ...
        return self_
            .get_ring()
            .normalize_map_back_from_order(&order_poly_ring, poly_ring, &result);
    }
        ...
        return result
            ...
            .map(|(f, k)| {
                (
                    self_
                        .get_ring()
                        .normalize_map_back_from_order(&order_poly_ring, poly_ring, &f),
                    k,
                )
            })
            .collect();
...
where
    ...
    P: RingStore + Copy,
    P::Ring: PolyRing + DivisibilityRing,
    BaseRing<P>: RingStore<Ring = NumberFieldByOrder<Impl, I>>,
{
    event!(
        Level::TRACE,
        poly_deg = poly_ring.degree(poly).unwrap_or(0),
        numberfield_deg = poly_ring.base_ring().rank()
    );
    ...
    'try_factor_directly: for attempt in 0..TRY_FACTOR_DIRECTLY_ATTEMPTS {
        ...
        if let Some(p) = inert_prime {
            ...
            let lc_poly = poly_ring.lc(poly).unwrap().clone();
            let monic_poly = evaluate_aX(poly_ring, poly, &lc_poly);
            let e = 2 * self_.get_ring().heuristic_exponent(
                &p,
            ...
            match factor_and_lift_mod_pe(poly_ring, &p, e, &monic_poly) {
                FactorAndLiftModpeResult::PartialFactorization(factorization) => {
                    debug_assert!(poly_ring.eq_el(
                        &monic_poly,
                        &poly_ring.normalize(poly_ring.prod(factorization.iter().map(|f| f.clone())))
                    ));
                    let result: Vec<_> = factorization
                        ...
                        .collect();
                    debug_assert!(poly_ring.eq_el(
                        &poly_ring.normalize(poly.clone()),
                        &poly_ring.normalize(
                            poly_ring.prod(result.iter().map(|(f, e)| poly_ring.pow(f.clone(), *e)))
                        )
                    ));
                ...
                FactorAndLiftModpeResult::NotSquarefreeModpe => {
                    ...
                    let power_decomposition = poly_power_decomposition_local(poly_ring, poly.clone());
...
where
    ...
    BaseRing<Impl>: RingStore<Ring = RationalFieldBase<I>>,
    I: RingStore,
    I::Ring: IntegerRing + ComputeResultantRing,
{
    #[instrument(skip_all, level = "trace")]
        ...
        BaseRing<P>: RingStore<Ring = Self>,
    {
        ...
        let mut to_factor = vec![(poly.clone(), 1)];
        ...
        while let Some((current, e_base)) = to_factor.pop() {
            if poly_ring.degree(&current).unwrap() == 1 {
                ...
                let poly_order = self_
                    .get_ring()
                    .scale_poly_to_order(poly_ring, &order_poly_ring, &current);
                ...
                match poly_factor_numberfield_heuristic(&order_poly_ring, &poly_order) {
                    ...
                    HeuristicFactorPolyInOrderResult::Unknown => result.extend(
                        poly_factor_extfield(&poly_ring, &current)
                            .0
                            .into_iter()
                            .map(|(f, e)| (f, e * e_base)),
                    ),
    ...
    I::Ring: IntegerRing,
{
    ...
    where
        P1: RingStore,
        P1::Ring: PolyRing,
        BaseRing<P1>: RingStore<Ring = NumberFieldBase<Impl, I>>,
        P2: RingStore,
        P2::Ring: PolyRing,
        BaseRing<P2>: RingStore<Ring = Self>,
        Self: 'ring,
    {
        debug_assert!(self.get_delegate() == from.base_ring().get_ring());
        debug_assert!(self.get_delegate() == to.base_ring().get_ring().get_delegate());
        let QQ = self.base.base_ring();
        let ZZ = QQ.base_ring();
        let denominator = QQ.inclusion().map(
            from.terms(poly)
                .map(|(c, _)| {
                    self.base
                        .wrt_canonical_basis(c)
                        .iter()
                        .map(|c| QQ.get_ring().den(&c).clone())
                        .fold(ZZ.one(), |a, b| ZZ.ideal_intersect(&a, &b))
                })
                .fold(ZZ.one(), |a, b| ZZ.ideal_intersect(&a, &b)),
        );
        debug_assert!(!QQ.is_zero(&denominator));
        return to.from_terms(from.terms(poly).map(|(c, i)| {
            (
                RingValue::from_ref(&self.base)
                    .into_inclusion()
                    .mul_ref_map(c, &denominator),
                i,
            )
        }));
    }
    ...
    where
        P1: RingStore,
        P1::Ring: PolyRing,
        BaseRing<P1>: RingStore<Ring = Self>,
        P2: RingStore,
        P2::Ring: PolyRing,
        BaseRing<P2>: RingStore<Ring = NumberFieldBase<Impl, I>>,
        Self: 'ring,
    {
        debug_assert!(self.get_delegate() == to.base_ring().get_ring());
        debug_assert!(self.get_delegate() == from.base_ring().get_ring().get_delegate());
        let result = to.from_terms(from.terms(poly).map(|(c, i)| (c.clone(), i)));
...
impl<Impl, I> Clone for NumberFieldByOrder<Impl, I>
where
    Impl: RingStore,
    Impl::Ring: Field + FreeAlgebra,
    BaseRing<Impl>: RingStore<Ring = RationalFieldBase<I>>,
...
where
    ...
    I::Ring: IntegerRing,
{
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result { write!(f, "NumberFieldByOrder({:?})", &self.base) }
}
...
where
    ...
    I::Ring: IntegerRing,
{
    type Base = NumberFieldBase<Impl, I>;
    type Element = El<NumberField<Impl, I>>;

    fn get_delegate(&self) -> &Self::Base { &self.base }
...
type LocalRing<'ring, I> = <<I as RingStore>::Ring as PolyLiftFactorsDomain>::LocalRing<'ring>;

type LocalRingPolyRing<'ring, I> =
    DensePolyRing<LocalRing<'ring, I>, DynConvolution<'ring, <LocalRing<'ring, I> as RingStore>::Ring>>;

type LocalField<'ring, I> = <<I as RingStore>::Ring as PolyLiftFactorsDomain>::LocalField<'ring>;

type LocalFieldPolyRing<'ring, I> =
    DensePolyRing<LocalField<'ring, I>, DynConvolution<'ring, <LocalField<'ring, I> as RingStore>::Ring>>;

type NumberFieldQuotientField<'ring, I> = AsFieldBase<
    FreeAlgebraImpl<
        <<I as RingStore>::Ring as IntegerPolyLiftFactorsDomain>::LocalFieldAsZn<'ring>,
        Vec<El<<<I as RingStore>::Ring as IntegerPolyLiftFactorsDomain>::LocalFieldAsZn<'ring>>>,
        DynConvolution<
            'ring,
            <<<I as RingStore>::Ring as IntegerPolyLiftFactorsDomain>::LocalFieldAsZn<'ring> as RingStore>::Ring,
        >,
    >,
>;
...
type NumberFieldQuotientRing<'ring, I> = FreeAlgebraImplBase<
    ...
    DynConvolution<'ring, <LocalRing<'ring, I> as RingStore>::Ring>,
>;
...
where
    ...
    I: 'ring,
{
    prime: <I::Ring as PolyLiftFactorsDomain>::SuitableIdeal<'ring>,
    ZZX: DensePolyRing<&'ring I, DynConvolution<'ring, I::Ring>>,
    number_field_poly: El<DensePolyRing<&'ring I, DynConvolution<'ring, I::Ring>>>,
    FpX: LocalFieldPolyRing<'ring, I>,
    Fp_as_ring: <I::Ring as PolyLiftFactorsDomain>::LocalRing<'ring>,
    Fp_as_zn: <I::Ring as IntegerPolyLiftFactorsDomain>::LocalFieldAsZn<'ring>,
    minpoly_factors_mod_p: Vec<El<LocalFieldPolyRing<'ring, I>>>,
}
...
where
    ...
    I: 'ring,
{
    fn lifted_factorization<'a>(
        &'a self,
        e: usize,
    ) -> (LocalRingPolyRing<'ring, I>, Vec<El<LocalRingPolyRing<'ring, I>>>) {
        let ZZX = &self.ZZX;
        let ZZ = ZZX.base_ring();
        let ZpeX = DensePolyRing::new(ZZ.get_ring().quotient_ring_at(&self.prime, e, 0), "X");
        let Zpe = ZpeX.base_ring();
        let FpX = &self.FpX;
        let Zpe_to_Fp = PolyLiftFactorsDomainIntermediateReductionMap::new(
            ZZ.get_ring(),
            &self.prime,
            Zpe,
            e,
...
where
    ...
    I::Ring: IntegerRing,
    I: 'ring,
    <I::Ring as PolyLiftFactorsDomain>::SuitableIdeal<'ring>: Debug,
{
    ...
    I::Ring: IntegerRing,
{
    type LocalRingBase<'ring>
        = NumberFieldQuotientRing<'ring, I>
    where
        Self: 'ring;

    type LocalRing<'ring>
        = RingValue<Self::LocalRingBase<'ring>>
    where
        Self: 'ring;

    type LocalFieldBase<'ring>
        = NumberFieldQuotientField<'ring, I>
    where
        Self: 'ring;

    type LocalField<'ring>
        = RingValue<Self::LocalFieldBase<'ring>>
    where
        Self: 'ring;

    type SuitableIdeal<'ring>
        = NumberRingIdeal<'ring, I>
    where
        Self: 'ring;
    ...
    fn heuristic_exponent<'ring, 'b, J>(
        &self,
    ...
    where
        J: Iterator<Item = &'b Self::Element>,
        Self: 'b,
        Self: 'ring,
    {
        ...
        let QQ = self.base.base_ring();
        let ZZ = QQ.base_ring();
        // to give any mathematically justifiable value, we would probably have to consider the canonical
        // norm; I don't want to deal with this here, so let's just use the coefficient norm
        // instead...
        let log2_max_coeff = coefficients
            .map(|c| {
                self.base
                    .wrt_canonical_basis(c)
                    .iter()
                    .map(|c| ZZ.abs_log2_ceil(QQ.get_ring().num(&c)).unwrap_or(0))
                    .max()
                    .unwrap()
            })
            .max()
            .unwrap_or(0);
        let log2_p = BigIntRing::RING
            .to_float_approx(&ZZ.get_ring().principal_ideal_generator(&ideal.prime))
            .log2();
        return ((log2_max_coeff as f64 + poly_deg as f64 + (self.rank() as f64).log2()) / log2_p
            * HEURISTIC_FACTOR_SIZE_OVER_POLY_SIZE_FACTOR)
            .ceil() as usize
            + 1;
    }
        ...
        F: FnMut() -> u64,
    {
        ...
        let ZZ = QQ.base_ring();
        let ZZX = DensePolyRing::new(ZZ, "X");
        let gen_poly = RingValue::from_ref(&self.base).generating_poly(
            &ZZX,
            LambdaHom::new(QQ, ZZ, |QQ, ZZ, x| {
                assert!(ZZ.is_one(QQ.get_ring().den(&x)));
                QQ.get_ring().num(&x).clone()
            }),
        );
        ...
        for _ in 0..MAX_PROBABILISTIC_REPETITIONS {
            ...
            let ZZ_to_Fp = LambdaHom::new(ZZ, Fp, |ZZ, Fp, x| {
                ZZ.get_ring().base_ring_to_field(
                    &p,
                    Fp_as_ring.get_ring(),
                    Fp.get_ring(),
                    0,
                    ZZ.get_ring()
                        .reduce_ring_el(&p, (Fp_as_ring.get_ring(), 1), 0, x.clone()),
                )
            });
            ...
            if factorization.iter().all(|(_, e)| *e == 1) {
                return NumberRingIdeal {
                    ...
                    Fp_as_zn: ZZ
                        .get_ring()
                        .local_field_into_zn(ZZ.get_ring().quotient_field_at(&p, 0)),
                    FpX,
                    ZZX,
                    Fp_as_ring,
                    prime: p,
    ...
    #[instrument(skip_all, level = "trace")]
    fn quotient_field_at<'ring>(
        &self,
        ideal: &Self::SuitableIdeal<'ring>,
        idx: usize,
    ) -> RingValue<NumberFieldQuotientField<'ring, I>>
    where
        Self: 'ring,
    {
        ...
        let mut x_pow_rank = (0..FpX.degree(irred_poly).unwrap())
            .map(|_| Fp_to_Fp_zn.codomain().zero())
            .collect::<Vec<_>>();
        ...
        let trailing_zeros = x_pow_rank
            .iter()
            .rev()
            .take_while(|x| Fp_to_Fp_zn.codomain().is_zero(x))
            .count();
        x_pow_rank.truncate(x_pow_rank.len() - trailing_zeros);
        let Fp: <<I as RingStore>::Ring as IntegerPolyLiftFactorsDomain>::LocalFieldAsZn<'ring> =
            ideal.Fp_as_zn.clone();
        return AsField::from(AsFieldBase::promise_is_perfect_field(FreeAlgebraImpl::new(
            Fp,
    ...
    fn quotient_ring_at<'ring>(
        ...
        e: usize,
        idx: usize,
    ) -> RingValue<NumberFieldQuotientRing<'ring, I>>
    where
        Self: 'ring,
    {
        assert!(idx < self.maximal_ideal_factor_count(ideal));
        ...
        let mut x_pow_rank = SparseMapVector::new(degree, ZZ.get_ring().quotient_ring_at(&ideal.prime, e, 0));
        for (c, i) in ZpeX.terms(irred_poly) {
            if i < x_pow_rank.len() {
                *x_pow_rank.at_mut(i) = Zpe.negate(c.clone());
            }
        }
        ...
        Self: 'ring,
    {
        ...
        let hom = to.base_ring().can_hom(ideal.FpX.base_ring()).unwrap().compose(
            PolyLiftFactorsDomainBaseRingToFieldIso::new(
                ZZ.get_ring(),
                &ideal.prime,
                from.base_ring().get_ring(),
                ideal.FpX.base_ring().get_ring(),
                0,
            ),
        );
        ...
        Self: 'ring,
    {
        ...
        assert!(idx < self.maximal_ideal_factor_count(ideal));
        let ZZ = self.base.base_ring().base_ring();
        let hom = PolyLiftFactorsDomainBaseRingToFieldIso::new(
            ZZ.get_ring(),
            &ideal.prime,
            to.base_ring().get_ring(),
            ideal.FpX.base_ring().get_ring(),
            0,
        )
        .inv()
        ...
        Self: 'ring,
    {
        ...
        let QQ = self.base.base_ring();
        let ZZ = QQ.base_ring();
        to.0.from_canonical_basis(from.0.wrt_canonical_basis(&x).iter().map(|c| {
            ZZ.get_ring().reduce_partial(
                &ideal.prime,
                (from.0.base_ring().get_ring(), from.1),
                (to.0.base_ring().get_ring(), to.1),
                0,
                c,
            )
        }))
        ...
        'ring: 'local + 'element,
    {
        ...
        let mut unit_vectors = (0..self.maximal_ideal_factor_count(ideal))
            .map(|_| ZpeX.zero())
            .collect::<Vec<_>>();
        product_except_one(
            &ZpeX,
        ...
        assert_el_eq!(
            &ZpeX,
            &complete_product,
            &RingValue::from_ref(&self.base).generating_poly(
                &ZpeX,
...
fn test_principal_ideal_ring_axioms() {
    ...
    crate::pid::generic_tests::test_principal_ideal_ring_axioms(&K, elements.iter().map(|x| x.clone()));
}
