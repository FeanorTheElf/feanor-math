    ...
    where Self::Type: MultivariatePolyRing
{
    ...
    fn from_terms<I>(&self, terms: I) -> El<Self>
        where I: Iterator<Item = (PolyCoeff<Self>, PolyMonomial<Self>)>
    {
...
pub mod generic_tests {
        ...
        where P::Type: MultivariatePolyRing
    {
        ...
        for i in 0..n {
            for a in &elements {
                let xi = ring.create_term(ring.base_ring().one(), ring.create_monomial((0..n).map(|k| if k == i { 1 } else { 0 })));
                assert!(!ring.is_zero(&ring.inclusion().mul_ref_map(&xi, a)));
            }
        }

        // test product
        if n >= 2 {
            let x0 = ring.create_term(ring.base_ring().one(), ring.create_monomial((0..n).map(|k| if k == 0 { 1 } else { 0 })));
            let x1 = ring.create_term(ring.base_ring().one(), ring.create_monomial((0..n).map(|k| if k == 1 { 1 } else { 0 })));
            ...
            for a in &elements {
                for b in &elements {
                    for c in &elements {
                        let f = ring.add(ring.inclusion().mul_ref_map(&x0, a), ring.inclusion().mul_ref_map(&x1, b));
                        let g = ring.add(ring.inclusion().mul_ref_map(&x0, c), ring.clone_el(&x1));
