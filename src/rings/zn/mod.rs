...
use crate::integer::*;
use crate::ordered::*;
use crate::pid::{EuclideanRingStore, PrincipalIdealRing, *};
...
///         let value = Zn.coerce(
///             Zn.integer_ring(),
///             int_cast(self.int_value, Zn.integer_ring(), &StaticRing::<i64>::RING),
///         );
    ...
    S::Type: ZnRing,
{
    ...
    pub fn any_preimage(&self, x: El<S>) -> El<R> {
        // the problem is that we don't know if `to.any_lift(x)` will fit into
        // `from.integer_ring()`; furthermore, profiling indicates that it won't help a lot
        // anyway, since taking the smallest lift now will usually make reduction cheaper
        // later
