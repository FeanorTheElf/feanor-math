...
pub trait MultivariatePolyRing: RingExtension {
        ...
        I: IntoIterator<Item = (El<Self::BaseRing>, Self::Monomial)>,
    {
        ...
        self.add_assign(
            lhs,
            self_ring.sum(rhs.into_iter().map(|(c, m)| self.create_term(c, m))),
        );
    }
        ...
        H: Homomorphism<<P::BaseRing as RingStore>::Type, <Self::BaseRing as RingStore>::Type>,
    {
        ...
        let mut exponents_storage = (0..self.indeterminate_count())
            .map(|_| 0)
            .collect::<Vec<_>>();
        return RingRef::new(self).from_terms(from.terms(el).map(|(c, m)| {
            ...
            (
                hom.map_ref(c),
                self.create_monomial(exponents_storage.iter().copied()),
            )
        }));
    }
    ...
    fn appearing_indeterminates(&self, f: &Self::Element) -> Vec<(usize, usize)> {
        let mut result = (0..self.indeterminate_count())
            .map(|_| 0)
            .collect::<Vec<_>>();
    ...
    fn monomial_mul(&self, lhs: Self::Monomial, rhs: &Self::Monomial) -> Self::Monomial {
        self.create_monomial(
            (0..self.indeterminate_count())
                .map(|i| self.exponent_at(&lhs, i) + self.exponent_at(rhs, i)),
        )
    }
    ...
    fn monomial_lcm(&self, lhs: Self::Monomial, rhs: &Self::Monomial) -> Self::Monomial {
        self.create_monomial(
            (0..self.indeterminate_count())
                .map(|i| max(self.exponent_at(&lhs, i), self.exponent_at(rhs, i))),
        )
    }
    ...
    fn monomial_div(
        ...
        let result = self.create_monomial((0..self.indeterminate_count()).map(|i| {
            if let Some(res) = self
                .exponent_at(&lhs, i)
                .checked_sub(self.exponent_at(rhs, i))
            {
                res
            } else {
                failed = true;
        ...
        V: VectorFn<R::Element>,
    {
        ...
        hom.codomain().sum(self.terms(f).map(|(c, m)| {
            hom.mul_map(
                hom.codomain().prod(
                    (0..self.indeterminate_count())
                        .map(|i| hom.codomain().pow(value.at(i), self.exponent_at(m, i))),
                ),
    ...
    fn specialize(&self, f: &Self::Element, var: usize, val: &Self::Element) -> Self::Element {
        ...
        for (c, m) in self.terms(f) {
            ...
            let new_m = self.create_monomial(
                (0..self.indeterminate_count())
                    .map(|i| if i == var { 0 } else { self.exponent_at(m, i) }),
            );
    ...
    Self::Type: MultivariatePolyRing,
{
        ...
        T: IntoIterator<Item = RingElementWrapper<&'a Self>>,
    {
        ...
        let wrapped_indets: [_; N] = std::array::from_fn(|i| {
            RingElementWrapper::new(
                self,
                self.create_term(self.base_ring().one(), self.indeterminate(i)),
            )
        });
        ...
        P::Type: MultivariatePolyRing,
    {
        ...
        for i in 0..n {
            for j in 0..n {
                ...
                let xixj_lcm =
                    ring.create_monomial((0..n).map(|k| if k == j || k == i { 1 } else { 0 }));
                assert_el_eq!(
                    ring,
        ...
        for i in 0..n {
            for a in &elements {
                let xi = ring.create_term(base_ring.one(), ring.indeterminate(i));
                assert!(
                    base_ring.is_zero(a) == ring.is_zero(&ring.inclusion().mul_ref_map(&xi, a))
                );
            }
        }
        ...
        for i in 0..n {
            let xi = ring.indeterminate(i);
            let mut a = ring.create_term(
                base_ring.int_hom().map(3),
                ring.create_monomial((0..n).map(|_| 0)),
            );
            ...
            ring.get_ring()
                .add_assign_from_terms(&mut a, terms_with_multiples);
            assert_el_eq!(
                &ring,
                ring.from_terms([
                    (
                        base_ring.int_hom().map(7),
                        ring.create_monomial((0..n).map(|_| 0))
                    ),
            ...
            for a in &elements {
                for b in &elements {
                    for c in &elements {
                        ...
                        let g =
                            ring.add(ring.inclusion().mul_ref_map(&x0_v, c), ring.clone_el(&x1_v));
                ...
                for b in &elements {
                    for c in &elements {
                        ...
                        let h = ring.from_terms([
                            ...
                            (base_ring.clone_el(b), ring.clone_monomial(&x0)),
                            (
                                base_ring.add_ref_fst(c, base_ring.one()),
                                ring.clone_monomial(&x0_2),
                            ),
                            (base_ring.one(), ring.clone_monomial(&x0x1)),
                        ]);
            ...
            for a in &elements {
                for b in &elements {
                    ...
                    assert_el_eq!(
                        ...
                        &ring.evaluate(
                            &f,
                            values.into_clone_ring_els(base_ring),
                            &base_ring.identity()
                        )
                    );
                }
