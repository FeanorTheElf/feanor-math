...
use crate::ring_properties::specialization::{FiniteRingSpecializable, FiniteRingOperation};
...
fn test_can_hom_axioms() {
    feanor_tracing::DelayedLogger::init_test();
    let from: DirectPowerRing<_, 3> = DirectPowerRing::new(ZZi64);
    let ring: DirectPowerRing<_, 3> = DirectPowerRing::new(Zn64B::new(3));
...
fn test_serialization() {
    feanor_tracing::DelayedLogger::init_test();
    let ring: DirectPowerRing<_, 3> = DirectPowerRing::new(Zn64B::new(3));
    crate::ring_properties::serialization::generic_tests::test_serialization(&ring, ring.elements());
}