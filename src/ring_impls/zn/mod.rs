...
pub trait ZnRing: PrincipalIdealRing + FiniteRing + CanHomFrom<Self::IntegerRingBase> {
    ...
    /// Computes the smallest positive lift for some `x` in `Z/nZ`, i.e. the smallest positive
    /// integer `m` such that `m = x mod n`.
    ///
    /// This will be one of `0, 1, ..., n - 1`. If an integer in `-(n - 1)/2, ..., -1, 0, 1, ..., (n
    /// - 1)/2` (for odd `n`) is needed instead, use [`ZnRing::smallest_lift()`].
...
pub mod generic_impls {
    ...
    ///  - the integer ring associated to the destination ring `Z/nZ` might not be large enough to
    ///    represent the input
    ///  - the destination ring might use Barett reductions (or similar) for fast modular reduction
    ///    if the input is bounded by some fixed bound `B`
    ...
    ///  - `to_large_int_ring`: an integer ring that can represent all integers for which we can
    ///    perform fast modular reduction (i.e. those bounded by `B`)
    ///  - `from_positive_representative_exact`: a function that performs the restricted reduction
    ///    `{0, ..., n - 1} -> Z/nZ`
    ///  - `from_positive_representative_bounded`: a function that performs the restricted reduction
    ///    `{0, ..., B - 1} -> Z/nZ`
    ///
    /// It first estimates the size of numbers by their bitlength, so don't use this for small
    /// integers (i.e. `ixx`-types), as the estimation is likely to take longer than the actual
    /// modular reduction.
    ///
    /// Note that the input size estimates consider only the bitlength of numbers, and so there is a
    /// small margin in which a reduction method for larger numbers than necessary is used.
    /// Furthermore, if the integer rings used can represent some but not all positive numbers of a
    /// certain bitlength, there might be rare edge cases with panics/overflows.
    ///
    /// In particular, if the input integer ring `Z` can represent the input `x`, but not `n` AND
    /// `x` and `n` have the same bitlength, this function might decide that we have to perform
    /// generic modular reduction (even though `x < n`), and try to map `n` into `Z`. This is never
    ...
    pub fn map_in_from_bigint<I: ?Sized + IntegerRing, J: ?Sized + IntegerRing, R: ?Sized + ZnRing, F, G>(
        ...
        to_large_int_ring: &J,
        el: I::Element,
        hom: &BigIntToZnHom<I, J, R>,
        from_positive_representative_exact: F,
        ...
        G: FnOnce(J::Element) -> R::Element,
    {
        ...
        let reduced = if highbit_el < hom.highbit_mod {
            ...
            from_positive_representative_exact(from.map_out(
                ZZ,
                from.euclidean_rem(n, &from.map_in_ref(ZZ, to.modulus(), &hom.hom)),
                &hom.iso,
            ))
        };
        ...
        R::Ring: 'conv + ZnRing + Field + SelfIso + CanHomFrom<StaticRingBase<i64>>,
    {
        ...
        let count = int_cast(count.try_into().unwrap(), ZZbig, ZZi64);
        let degree = int_bisect::find_root_floor(ZZi64, 1, |d| {
...
///     }
/// }
/// choose_zn_impl(ZZi64, 17, DoStuff { int_value });
/// ```
    ...
    F: ZnOperation,
{
    if ZZ.abs_highest_set_bit(&n).unwrap_or(0) < 57 {
        f.call(zn_64b::Zn64B::new(ZZi64.coerce(&ZZ, n) as u64))
    } else {
        f.call(zn_big::ZnGB::new(ZZbig, int_cast(n, &ZZbig, &ZZ)))
    }
}
    ...
    S::Ring: ZnRing,
{
    pub fn new(from: R, to: S) -> Option<Self> {
        let from_char = from.characteristic(&ZZbig).unwrap();
        let to_char = to.characteristic(&ZZbig).unwrap();
        if let Some(frac) = ZZbig.checked_div(&from_char, &to_char) {
            let map_forward_requirement: ReductionMapRequirements =
                if to.integer_ring().get_ring().representable_bits().is_none()
                    || ZZbig.is_lt(
                        &from_char,
                        &ZZbig.power_of_two(to.integer_ring().get_ring().representable_bits().unwrap()),
                    )
                {
            ...
            Some(Self {
                ...
                from_from_int: from
                    ...
                    .unwrap(),
                fraction_of_quotients: from.can_hom(from.integer_ring()).unwrap().map(int_cast(
                    frac,
                    from.integer_ring(),
                    ZZbig,
                )),
    ...
    ///     Z25,
    ///     Z25.int_hom().map(15),
    ///     f.mul_quotient_fraction(Z5.int_hom().map(3))
    /// );
    /// ```
    ...
    ///     Z25,
    ///     Z25.int_hom().map(-2),
    ///     f.smallest_lift(Z5.int_hom().map(3))
    /// );
    /// ```
    pub fn smallest_lift(&self, x: El<S>) -> El<R> {
        self.from.get_ring().map_in(
            self.from.integer_ring().get_ring(),
            int_cast(
                self.to.smallest_lift(x),
                self.from.integer_ring(),
                self.to.integer_ring(),
            ),
            &self.from_from_int,
        )
    }
    ...
    S::Ring: ZnRing,
{
    ...
    fn map(&self, x: El<R>) -> El<S> {
        let value = match self.map_forward_requirement {
            ...
            ReductionMapRequirements::ExplicitReduce => self
                .from
                .integer_ring()
                .euclidean_rem(self.from.any_lift(x), &self.to_modulus),
        };
        self.to.get_ring().map_in(
            self.to.integer_ring().get_ring(),
            int_cast(value, self.to.integer_ring(), self.from.integer_ring()),
            &self.to_from_int,
        )
    }
...
fn test_reduction_map_large_value() {
    ...
    let ring2 = zn_big::ZnGB::new(ZZbig, ZZbig.power_of_two(666));
...
fn test_reduction_map() {
    ...
    let ring2 = zn_big::ZnGB::new(ZZbig, ZZbig.int_hom().map(65537 * 3));
