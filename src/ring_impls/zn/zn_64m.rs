...
impl Zn64MBase {
    ...
    fn repr_bound(&self) -> u64 { self.modulus_times_three }
...
impl Zn64M {
    #[stability::unstable(feature = "enable")]
    pub const fn new(modulus: u64) -> Self { RingValue::from(Zn64MBase::new(modulus)) }
}
...
impl RingBase for Zn64MBase {
    ...
    fn zero(&self) -> Self::Element { Zn64MEl(0) }
    ...
    I::Ring: IntegerRing,
{
    ...
    fn map_in(
        ...
        self.from_int_promise_reduced(int_cast(
...
impl<I: ?Sized + IntegerRing> CanHomFrom<I> for Zn64MBase {
    ...
    default fn has_canonical_hom(&self, from: &I) -> Option<Self::Homomorphism> {
        super::generic_impls::has_canonical_hom_from_bigint(from, self, ZZi64.get_ring(), None)
    }
    ...
    for n in TEST_MODULI {
        if n < 100 {
            ...
            for k in 0..=ring.get_ring().repr_bound() {
                assert_eq!(
                    k as i64 % n as i64,
                    ((ring.smallest_positive_lift(Zn64MEl(k)) as i128 * (1 << 64)) % n as i128) as i64
                );
            }
        }
    }
