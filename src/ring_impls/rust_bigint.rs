...
impl<A: Allocator + Send + Sync + Clone> RustBigIntRingBase<A> {
    ...
    #[stability::unstable(feature = "enable")]
    pub fn fma_i64_with_shift(
        &self,
        lhs: &RustBigInt<A>,
        rhs: i64,
        mut summand: RustBigInt<A>,
        shift: u32,
    ) -> RustBigInt<A> {
...
impl<A: Allocator + Send + Sync + Clone> RingBase for RustBigIntRingBase<A> {
    ...
    fn is_zero(&self, value: &Self::Element) -> bool { effective_length(&value.1) == 0 }
    ...
    fn fmt_el_within<'a>(
        ...
        let mut remainders: Vec<u64> = Vec::with_capacity(effective_length(&value.1) * u64::BITS as usize / 3);
...
impl<A1: Allocator + Send + Sync + Clone, A2: Allocator + Send + Sync + Clone> IntCast<RustBigIntRingBase<A2>>
...
impl<A: Allocator + Send + Sync + Clone> OrderedRing for RustBigIntRingBase<A> {
    fn cmp(&self, lhs: &Self::Element, rhs: &Self::Element) -> Ordering {
        match (lhs.0, rhs.0) {
            ...
            (..) if self.is_zero(lhs) && self.is_zero(rhs) => Ordering::Equal,
...
impl<A: Allocator + Send + Sync + Clone> PrincipalIdealRing for RustBigIntRingBase<A> {
    ...
    fn extended_ideal_gen(
        ...
        let from_i64 = RingValue::from_ref(self).can_hom(&ZZi64).unwrap();
            ...
            if i_a - i_b >= 16 {
                ...
                let apply_transform = |[a, b]: [&mut RustBigInt<A>; 2]| {
                    ...
                    *a = from_i64.fma_map(
                        &b,
                        &transform[1],
                        from_i64.mul_map(replace(a, self.zero()), transform[0]),
                    );
                    *b = new_b;
                };
...
impl Serialize for RustBigIntRingBase<Global> {
    ...
    where
        S: Serializer,
    {
        SerializableNewtypeStruct::new("RustBigIntRing", ()).serialize(serializer)
    }
}
...
impl<'de> Deserialize<'de> for RustBigIntRingBase<Global> {
    ...
    where
        D: Deserializer<'de>,
    {
        DeserializeSeedNewtypeStruct::new("RustBigIntRing", PhantomData::<()>)
...
impl<A: Allocator + Send + Sync + Clone> CanHomFrom<StaticRingBase<i64>> for RustBigIntRingBase<A> {
    fn fma_map_in(
        &self,
        _: &StaticRingBase<i64>,
        lhs: &RustBigInt<A>,
        rhs: &<StaticRingBase<i64> as RingBase>::Element,
        summand: RustBigInt<A>,
        (): &(),
    ) -> RustBigInt<A> {
    ...
    fn mul_assign_map_in(
        &self,
        from: &StaticRingBase<i64>,
        lhs: &mut RustBigInt<A>,
        rhs: <StaticRingBase<i64> as RingBase>::Element,
        (): &(),
    ) {
...
impl<A: Allocator + Send + Sync + Clone> IntegerRing for RustBigIntRingBase<A> {
    fn to_float_approx(&self, value: &Self::Element) -> f64 {
        ...
        match effective_length(&value.1) {
            ...
            l => {
                (value.1[l - 1] as f64 * 2.0f64.powi(((l - 1) * u64::BITS as usize).try_into().unwrap())
                    + value.1[l - 2] as f64 * 2.0f64.powi(((l - 2) * u64::BITS as usize).try_into().unwrap()))
                    * sign
            }
        }
    }
...
fn test_print_power_2() {
    ...
    assert_eq!(
        ...
        format!("{}", RustBigIntRing::RING.formatted_el(&x))
    );
}
...
fn test_bigint_divisibility_ring_axioms() {
    feanor_tracing::DelayedLogger::init_test();
    crate::ring_properties::divisibility::generic_tests::test_divisibility_axioms(ZZ, edge_case_elements())
}

#[test]
fn test_bigint_euclidean_ring_axioms() {
    feanor_tracing::DelayedLogger::init_test();
    crate::ring_properties::pid::generic_tests::test_euclidean_ring_axioms(ZZ, edge_case_elements());
}

#[test]
fn test_bigint_principal_ideal_ring_axioms() {
    feanor_tracing::DelayedLogger::init_test();
    crate::ring_properties::pid::generic_tests::test_principal_ideal_ring_axioms(ZZ, edge_case_elements());
}

#[test]
fn test_bigint_integer_ring_axioms() {
    feanor_tracing::DelayedLogger::init_test();
    crate::ring_properties::integer::generic_tests::test_integer_axioms(ZZ, edge_case_elements())
}
...
fn test_get_uniformly_random() {
    feanor_tracing::DelayedLogger::init_test();
    crate::ring_properties::integer::generic_tests::test_integer_get_uniformly_random(ZZ);
...
fn test_canonical_iso_static_int() {
    feanor_tracing::DelayedLogger::init_test();
    // for the hom test, we have to be able to multiply elements in `StaticRing::<i128>::RING`, so we
    // cannot test `i128::MAX` or `i128::MIN`
...
fn test_fma_map_in() {
    feanor_tracing::DelayedLogger::init_test();
    let from_i64 = ZZ.can_hom(&ZZi64).unwrap();
    assert_el_eq!(
        ZZ,
        ZZ.get_ring().parse("10000000000000000000000", 10).unwrap(),
        from_i64.fma_map(
    ...
    assert_el_eq!(
        ZZ,
        ZZ.get_ring().parse("10100000000000000000000", 10).unwrap(),
        from_i64.fma_map(
            &ZZ.get_ring().parse("100000000000000000000", 10).unwrap(),
            &100,
            ZZ.get_ring().parse("100000000000000000000", 10).unwrap()
    ...
    assert_el_eq!(
        ZZ,
        ZZ.get_ring().parse("-9900000000000000000000", 10).unwrap(),
        from_i64.fma_map(
            &ZZ.get_ring().parse("100000000000000000000", 10).unwrap(),
            &-100,
            ZZ.get_ring().parse("100000000000000000000", 10).unwrap()
...
fn test_serialize() {
    feanor_tracing::DelayedLogger::init_test();
    crate::ring_properties::serialization::generic_tests::test_serialization(ZZ, edge_case_elements())
}
