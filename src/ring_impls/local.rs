...
fn test_canonical_hom_axioms_static_int() {
    let R = AsLocalPIR::from_zn(Zn::new(ZZi64, 8)).unwrap();
    crate::ring::generic_tests::test_hom_axioms(ZZi64, &R, 0..8);
}

#[test]
fn test_divisibility_axioms() {
    let R = AsLocalPIR::from_zn(Zn::new(ZZi64, 8)).unwrap();
    crate::ring_properties::divisibility::generic_tests::test_divisibility_axioms(&R, R.elements());
}

#[test]
fn test_principal_ideal_ring_axioms() {
    let R = AsLocalPIR::from_zn(Zn::new(ZZi64, 8)).unwrap();
    crate::ring_properties::pid::generic_tests::test_principal_ideal_ring_axioms(&R, R.elements());
    let R = AsLocalPIR::from_zn(Zn::new(ZZi64, 9)).unwrap();
    crate::ring_properties::pid::generic_tests::test_principal_ideal_ring_axioms(&R, R.elements());
    let R = AsLocalPIR::from_zn(Zn::new(ZZi64, 17)).unwrap();
    crate::ring_properties::pid::generic_tests::test_principal_ideal_ring_axioms(&R, R.elements());
}
...
fn test_checked_div_min() {
    let ring = AsLocalPIR::from_zn(Zn::new(ZZi64, 27)).unwrap();
    assert_el_eq!(
        &ring,
        ring.zero(),
