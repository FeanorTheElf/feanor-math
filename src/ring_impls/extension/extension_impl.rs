...
/// let ring = MonogeneticExtensionImpl::new(base_ring, vec![-1, 1]);
...
/// Type of an element of [`MonogeneticExtensionImpl`].
...
impl<'conv, R> MonogeneticExtensionImpl<R, SparsePolyModulus<R>, DynConvolution<'conv, R::Ring>, Global>
where
    R: RingStore + 'conv,
{
    /// Creates a new [`MonogeneticExtensionImpl`] ring as extension of the given base ring.
    ///
    /// For details, see [`MonogeneticExtensionImplBase::new()`].
    pub fn new(base_ring: R, x_pow_rank: Vec<El<R>>) -> Self {
        RingValue::from(MonogeneticExtensionImplBase::new(base_ring, x_pow_rank))
    }
}

impl<'conv, R> MonogeneticExtensionImplBase<R, SparsePolyModulus<R>, DynConvolution<'conv, R::Ring>, Global>
where
    R: RingStore + 'conv,
{
    /// Creates a new [`MonogeneticExtensionImpl`] ring as extension of the given base ring.
...
{
    /// Creates a new [`MonogeneticExtensionImpl`] ring, using the given polynomial modulus.
    ///
    /// For details, see [`MonogeneticExtensionImplBase::new()`].
    ...
    C: ConvolutionAlgorithm<R::Ring>,
{
    ...
    pub fn create(modulus: M, gen_name: &'static str, element_allocator: A, convolution: C) -> Self {
        ...
        assert!(rank >= 1);
        assert!(modulus.supported_operand_degree() >= rank * 2 - 1);
    ...
    C: ConvolutionAlgorithm<R::Ring>,
{
    ...
    fn fma(&self, lhs: &Self::Element, rhs: &Self::Element, summand: Self::Element) -> Self::Element {
        ...
        self.modulus.perform_reduction(&mut tmp[..(2 * self.rank())]);
    ...
    C: ConvolutionAlgorithm<R::Ring>,
{
    ...
    fn canonical_gen(&self) -> Self::Element {
        if self.rank() == 1 {
            self.from_canonical_basis([self.modulus().x_pow_rank().at(0).clone()])
        } else {
            self.from_canonical_basis((0..self.rank()).map(|i| {
                if i == 1 {
                    self.base_ring().one()
                } else {
                    self.base_ring().zero()
    ...
    R2::Ring: CanIsoFromTo<R1::Ring>,
{
    ...
    fn map_out(
        ...
        from.from_canonical_basis((0..self.rank()).map(|i| {
            self.base_ring()
                .get_ring()
...
#[cfg(test)]
fn test_ring0_and_elements() -> (MonogeneticExtensionImpl<Zn64B>, Vec<MonogeneticExtensionImplEl<Zn64B>>) {
    let R = MonogeneticExtensionImpl::new(Zn64B::new(7), vec![Zn64B::new(7).neg_one()]);
...
fn test_ring1_and_elements() -> (
    MonogeneticExtensionImpl<StaticRing<i64>>,
    Vec<MonogeneticExtensionImplEl<StaticRing<i64>>>,
) {
    let ZZ = ZZi64;
    let R = MonogeneticExtensionImpl::new(ZZ, vec![1, 1]);
...
fn test_ring2_and_elements() -> (
    ...
    let ZZ = ZZi64;
    let R = MonogeneticExtensionImpl::new(ZZ, vec![1, -1, 1]);
    let mut elements = Vec::new();
    for a in [-2, 0, 1, 2] {
        for b in [0, 2] {
            for c in [0, 2] {
...
fn test_ring3_and_elements() -> (
    ...
    let ZZ = ZZi64;
    let R = MonogeneticExtensionImpl::new(ZZ, vec![1, 0, 0]);
    let mut elements = Vec::new();
    for a in [-2, 0, 1, 2] {
        for b in [0, 2] {
            for c in [0, 2] {
...
fn test_ring4_and_elements() -> (
    MonogeneticExtensionImpl<StaticRing<i64>, SchoolbookPolyModulus<StaticRing<i64>>>,
    Vec<MonogeneticExtensionImplEl<StaticRing<i64>>>,
) {
    let ZZ = ZZi64;
    let R = MonogeneticExtensionImpl::new_with_modulus(SchoolbookPolyModulus::new(ZZ, vec![0, 1, 0]));
    let mut elements = Vec::new();
    for a in [-2, 0, 1, 2] {
        for b in [0, 2] {
            for c in [0, 2] {
...
fn test_ring_axioms() {
    ...
    let ring = MonogeneticExtensionImpl::new(base_ring, x_pow_rank);
    let mut rng = oorandom::Rand64::new(0);
    let els =
        (0..10).map(|_| ring.from_canonical_basis((0..64).map(|_| ring.base_ring().random_element(|| rng.rand_u64()))));
...
fn test_rank_1_ring() {
    ...
    let ring = MonogeneticExtensionImpl::new(base_ring, vec![base_ring.int_hom().map(1)])
        .as_field()
        .ok()
        .unwrap();
...
fn test_division() {
    ...
    let ring = MonogeneticExtensionImpl::new(base_ring, vec![i.map(-1), i.map(-1)]);

    assert_el_eq!(
        ring,
        &ring.from_canonical_basis([i.map(1), i.map(3)]),
        &ring
            .checked_div(&ring.one(), &ring.from_canonical_basis([i.map(2), i.map(3)]))
            .unwrap()
    );
...
fn test_division_ring_of_integers() {
    feanor_tracing::DelayedLogger::init_test();
    let base_ring = ZZi64;
    let ring = MonogeneticExtensionImpl::new(base_ring, vec![11, 0]);
    ...
    assert!(
        ring.checked_div(&ring.from_canonical_basis([2, 0]), &ring.from_canonical_basis([3, 0]))
            .is_none()
    );
}
...
fn test_cubic_mul() {
    ...
    let ring = MonogeneticExtensionImpl::new(base_ring, vec![modulo.map(-1), modulo.map(-1), modulo.map(-1)]);
...
fn test_as_field() {
    ...
    let ring = MonogeneticExtensionImpl::new(base_ring, vec![base_ring.int_hom().map(2), base_ring.zero()])
        .as_field()
        .ok()
        .unwrap();
    let ring_ref = &ring;
    crate::ring_properties::field::generic_tests::test_field_axioms(
        &ring,
        (0..3).flat_map(|x| {
            (0..3).map(move |y| ring_ref.from_canonical_basis([base_ring.int_hom().map(x), base_ring.int_hom().map(y)]))
        }),
    );

    assert!(
        MonogeneticExtensionImpl::new(base_ring, vec![base_ring.int_hom().map(1), base_ring.zero()])
            .as_field()
            .is_err()
    );
}
