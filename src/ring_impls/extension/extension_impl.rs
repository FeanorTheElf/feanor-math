...
/// ```
pub struct FreeAlgebraImplBase<
    R,
    V = SparseMapVector<R>,
...
/// [`FreeAlgebraImplBase`].
pub type FreeAlgebraImpl<R, V = SparseMapVector<R>, C = DynConvolution<'static, <R as RingStore>::Ring>, A = Global> =
    RingValue<FreeAlgebraImplBase<R, V, C, A>>;
    ...
    V: VectorView<El<R>> + Send + Sync + Clone,
{
    ...
    ///
    /// For details, see [`FreeAlgebraImplBase::new()`].
    pub fn new(base_ring: R, rank: usize, x_pow_rank: V) -> Self {
        RingValue::from(FreeAlgebraImplBase::new(base_ring, rank, x_pow_rank))
    }
}

impl<'conv, R, V> FreeAlgebraImplBase<R, V, DynConvolution<'conv, R::Ring>, Global>
where
    R: RingStore + 'conv,
    V: VectorView<El<R>> + Send + Sync,
{
    /// Creates a new [`FreeAlgebraImplBase`] ring as extension of the given base ring.
    ///
    ...
    pub fn new(base_ring: R, rank: usize, x_pow_rank: V) -> Self {
        let log2_padded_len = ZZi64
            ...
            .unwrap();
        let convolution = <R::Ring>::create_default_convolution(base_ring.clone(), Some(2 << log2_padded_len));
    ...
    C: ConvolutionAlgorithm<R::Ring>,
{
    ...
    pub fn new_with_convolution(
        ...
        FreeAlgebraImplBase::create(base_ring, rank, x_pow_rank, gen_name, element_allocator, convolution)
    }
    ...
    ) -> FreeAlgebraImplBase<R, V, C_new, A> {
        FreeAlgebraImplBase {
    ...
    pub fn with_allocator<A_new: Allocator + Clone + Send + Sync>(
        self,
        new_allocator: A_new,
    ) -> FreeAlgebraImplBase<R, V, C, A_new> {
        FreeAlgebraImplBase {
            base_ring: self.base_ring,
            x_pow_rank: self.x_pow_rank,
    ...
    pub fn x_pow_rank(&self) -> &V { &self.x_pow_rank }
    ...
    fn reduce_mod_poly(&self, data: &mut [El<R>]) {
        ...
        V::specialize_sparse(ReduceModulo {
            base_ring: self.base_ring(),
            rank: self.rank,
            x_pow_rank: &self.x_pow_rank,
            data,
    ...
    C: ConvolutionAlgorithm<R::Ring>,
{
    fn eq(&self, other: &Self) -> bool {
        self.base_ring().get_ring() == other.base_ring().get_ring()
            ...
            && (0..self.rank()).all(|i| {
                (i >= self.x_pow_rank.len() && i >= other.x_pow_rank.len())
...
impl<'a, R, V> Serialize for FreeAlgebraImplBase<R, V, DynConvolution<'a, R::Ring>, Global>
where
...
impl<'de, 'conv, R> Deserialize<'de>
    for FreeAlgebraImplBase<R, SparseMapVector<R>, DynConvolution<'conv, R::Ring>, Global>
where
    ...
    R::Ring: SerializableElementRing,
{
        ...
        D: Deserializer<'de>,
    {
        ...
        for (c, i) in poly_ring.terms(&poly) {
            *x_pow_rank.at_mut(i) = poly_ring.base_ring().negate(c.clone());
        }
        ...
        let log2_padded_len = ZZi64
            ...
            .unwrap();
        let convolution = <R::Ring>::create_default_convolution(base_ring.clone(), Some(2 << log2_padded_len));
        return Ok(FreeAlgebraImplBase::new_with_convolution(
            base_ring,
            rank,
            x_pow_rank,
            "θ",
...
impl<'de, 'conv, R> Deserialize<'de> for FreeAlgebraImplBase<R, Vec<El<R>>, DynConvolution<'conv, R::Ring>, Global>
where
    ...
    R::Ring: SerializableElementRing,
{
        ...
        D: Deserializer<'de>,
    {
        ...
        let convolution = <R::Ring>::create_default_convolution(base_ring.clone(), Some(2 << log2_padded_len));
        return Ok(
            FreeAlgebraImplBase::new_with_convolution(base_ring, rank, x_pow_rank, "θ", Global, convolution).into(),
        );
    }
}
    ...
    C: ConvolutionAlgorithm<R::Ring>,
{
    ...
    fn canonical_gen(&self) -> Self::Element {
        if self.rank() == 1 {
            if self.x_pow_rank.len() == 1 {
                self.from_canonical_basis([self.x_pow_rank.at(0).clone()])
            } else {
    ...
    fn mul_assign_gen_power(&self, el: &mut Self::Element, power: usize) {
        ...
        gen_pow_d.extend(self.x_pow_rank.as_iter().map(|c| c.clone()));
    ...
    C: ConvolutionAlgorithm<R::Ring>,
{
    fn specialize<O: FiniteRingOperation<Self>>(op: O) -> O::Output {
        ...
        {
            operation: O,
            ring: PhantomData<FreeAlgebraImplBase<R, V, C, A>>,
        }

        impl<R, V, C, A, O> FiniteRingOperation<R::Ring> for OpWrapper<R, V, C, A, O>
        where
            R: RingStore,
            R::Ring: FiniteRingSpecializable,
            V: VectorView<El<R>> + Send + Sync,
...
impl_field_wrap_unwrap_homs! {
    <{R1, V1, C1, A1, R2, V2, C2, A2}> FreeAlgebraImplBase<R1, V1, C1, A1>, FreeAlgebraImplBase<R2, V2, C2, A2>
        where R1: RingStore, R1::Ring: LinSolveRing, V1: VectorView<El<R1>> + Send + Sync, A1: Allocator + Clone + Send + Sync, C1: ConvolutionAlgorithm<R1::Ring>,
            R2: RingStore, R2::Ring: LinSolveRing, V2: VectorView<El<R2>> + Send + Sync, A2: Allocator + Clone + Send + Sync, C2: ConvolutionAlgorithm<R2::Ring>,
...
impl<R1, V1, C1, A1, R2, V2, C2, A2> CanHomFrom<FreeAlgebraImplBase<R1, V1, C1, A1>>
    for FreeAlgebraImplBase<R2, V2, C2, A2>
where
    R1: RingStore,
    V1: VectorView<El<R1>> + Send + Sync,
    A1: Allocator + Clone + Send + Sync,
    C1: ConvolutionAlgorithm<R1::Ring>,
    R2: RingStore,
    V2: VectorView<El<R2>> + Send + Sync,
    ...
    R2::Ring: CanHomFrom<R1::Ring>,
{
    ...
    fn map_in(
        &self,
        from: &FreeAlgebraImplBase<R1, V1, C1, A1>,
        el: <FreeAlgebraImplBase<R1, V1, C1, A1> as RingBase>::Element,
...
impl<R1, V1, C1, A1, R2, V2, C2, A2> CanIsoFromTo<FreeAlgebraImplBase<R1, V1, C1, A1>>
    for FreeAlgebraImplBase<R2, V2, C2, A2>
where
    R1: RingStore,
    V1: VectorView<El<R1>> + Send + Sync,
    ...
    R2::Ring: CanIsoFromTo<R1::Ring>,
{
    ...
    fn map_out(
        &self,
        from: &FreeAlgebraImplBase<R1, V1, C1, A1>,
        el: <FreeAlgebraImplBase<R2, V2, C2, A2> as RingBase>::Element,
        iso: &Self::Isomorphism,
    ) -> <FreeAlgebraImplBase<R1, V1, C1, A1> as RingBase>::Element {
...
#[cfg(test)]
use crate::ring_impls::zn::zn_64b::{Zn64B, Zn64BEl};
#[cfg(test)]
...
fn test_ring1_and_elements() -> (
    FreeAlgebraImpl<StaticRing<i64>, [i64; 2]>,
    Vec<FreeAlgebraImplEl<StaticRing<i64>>>,
) {
    let ZZ = ZZi64;
...
fn test_ring2_and_elements() -> (
    FreeAlgebraImpl<StaticRing<i64>, Vec<i64>>,
    Vec<FreeAlgebraImplEl<StaticRing<i64>>>,
) {
    let ZZ = ZZi64;
...
fn test_ring4_and_elements() -> (
    FreeAlgebraImpl<StaticRing<i64>, SparseMapVector<StaticRing<i64>>>,
    Vec<FreeAlgebraImplEl<StaticRing<i64>>>,
) {
    let ZZ = ZZi64;
    // ZZ[X]/(X^3 - X)
    let mut modulus = SparseMapVector::new(3, ZZi64);
    *modulus.at_mut(1) = 1;
...
fn test_rank_1_ring() {
    ...
    let ring = FreeAlgebraImpl::new(base_ring, 1, [base_ring.int_hom().map(1)])
        .as_field()
        .ok()
        .unwrap();
...
fn test_as_field() {
    ...
    let ring = FreeAlgebraImpl::new(base_ring, 2, [base_ring.int_hom().map(2)])
        .as_field()
        .ok()
        .unwrap();
    let ring_ref = &ring;
    ...
    assert!(
        FreeAlgebraImpl::new(base_ring, 2, [base_ring.int_hom().map(1)])
            .as_field()
            .is_err()
    );
}
...
fn test_serialize_deserialize() {
    feanor_tracing::DelayedLogger::init_test();
    crate::ring_properties::serialization::generic_tests::test_serialize_deserialize(test_ring0_and_elements().0.into());
    crate::ring_properties::serialization::generic_tests::test_serialize_deserialize(test_ring2_and_elements().0.into());
    crate::ring_properties::serialization::generic_tests::test_serialize_deserialize(test_ring3_and_elements().0.into());
}
