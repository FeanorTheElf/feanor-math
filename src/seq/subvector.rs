...
impl<V: VectorView<T>, T: ?Sized> VectorView<T> for SubvectorView<V, T> {
    ...
    fn specialize_sparse<Op: SparseVectorViewOperation<T, Self>>(op: Op) -> Op::Output {
        ...
        impl<T: ?Sized, V: VectorView<T>, Op: SparseVectorViewOperation<T, SubvectorView<V, T>>>
            SparseVectorViewOperation<T, V> for WrapSubvector<T, V, Op>
        {
            type Output = Op::Output;

            fn execute(self) -> Self::Output
            where
                V: VectorViewSparse<T>,
            {
                self.op.execute()
            }
        ...
        V::specialize_sparse(WrapSubvector {
            op,
            element: PhantomData,
            vector: PhantomData,
        })
...
fn test_subvector_sparse() {
    ...
    *sparse_vector.at_mut(257) = 257;
    ...
    impl SparseVectorViewOperation<i64, SubvectorView<SparseMapVector<StaticRing<i64>>, i64>> for Verify {
        ...
        fn execute(self) -> Self::Output
        where
            SubvectorView<SparseMapVector<StaticRing<i64>>, i64>: VectorViewSparse<i64>,
        {
            assert!(
        ...
        fn fallback(self) -> Self::Output { unreachable!() }
    }
