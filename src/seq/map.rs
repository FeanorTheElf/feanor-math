...
use crate::function::{IdentityFunction, TensorProductFunction};
use crate::impl_specialize_sparse_wrapped_vector;
use crate::seq::VectorViewSparse;
...
impl<V: Clone + VectorView<T>, T: ?Sized, U: ?Sized, F: Clone + for<'a> Fn(&'a T) -> &'a U> Clone
    for VectorViewMap<V, T, U, F>
{
    fn clone(&self) -> Self {
        Self {
...
impl<V: Copy + VectorView<T>, T: ?Sized, U: ?Sized, F: Copy + for<'a> Fn(&'a T) -> &'a U> Copy
    for VectorViewMap<V, T, U, F>
{
}
...
impl<V: VectorView<T>, T: ?Sized, U: ?Sized, F: for<'a> Fn(&'a T) -> &'a U> VectorView<U>
    for VectorViewMap<V, T, U, F>
{
    fn at(&self, i: usize) -> &U { (self.mapping_fn)(self.base.at(i)) }
...
impl<V: VectorViewSparse<T>, T: ?Sized, U: ?Sized, F: for<'a> Fn(&'a T) -> &'a U> VectorViewSparse<U>
    for VectorViewMap<V, T, U, F>
{
    type Iter<'a>
        = std::iter::Map<V::Iter<'a>, TensorProductFunction<IdentityFunction, &'a F>>
    where
        Self: 'a,
        U: 'a;
    ...
    fn nontrivial_entries<'a>(&'a self) -> Self::Iter<'a> {
        self.base
            .nontrivial_entries()
            .map(TensorProductFunction(IdentityFunction, &self.mapping_fn))
    }
}

pub struct VectorViewMapMut<
    V: VectorViewMut<T>,
    T: ?Sized,
    U: ?Sized,
...
impl<
    ...
    F_const: for<'a> Fn(&'a T) -> &'a U,
    F_mut: for<'a> FnMut(&'a mut T) -> &'a mut U,
> VectorViewMapMut<V, T, U, F_const, F_mut>
{
...
impl<
    ...
    F_const: for<'a> Fn(&'a T) -> &'a U,
    F_mut: for<'a> FnMut(&'a mut T) -> &'a mut U,
> VectorView<U> for VectorViewMapMut<V, T, U, F_const, F_mut>
{
    fn at(&self, i: usize) -> &U { (self.mapping_fns.0)(self.base.at(i)) }
...
impl<
    ...
    F_const: for<'a> Fn(&'a T) -> &'a U,
    F_mut: for<'a> FnMut(&'a mut T) -> &'a mut U,
> VectorViewMut<U> for VectorViewMapMut<V, T, U, F_const, F_mut>
{
    fn at_mut(&mut self, i: usize) -> &mut U { (self.mapping_fns.1)(self.base.at_mut(i)) }
}
...
impl<
    ...
    F_const: for<'a> Fn(&'a T) -> &'a U,
    F_mut: for<'a> FnMut(&'a mut T) -> &'a mut U,
> SwappableVectorViewMut<U> for VectorViewMapMut<V, T, U, F_const, F_mut>
{
    fn swap(&mut self, i: usize, j: usize) { self.base.swap(i, j) }
}

impl<
    V: VectorViewSparse<T> + VectorViewMut<T>,
    T: ?Sized,
    U: ?Sized,
    F_const: for<'a> Fn(&'a T) -> &'a U,
    F_mut: for<'a> FnMut(&'a mut T) -> &'a mut U,
> VectorViewSparse<U> for VectorViewMapMut<V, T, U, F_const, F_mut>
{
    type Iter<'a>
        = std::iter::Map<V::Iter<'a>, TensorProductFunction<IdentityFunction, &'a F_const>>
    where
        Self: 'a,
        U: 'a;
    ...
    fn nontrivial_entries<'a>(&'a self) -> Self::Iter<'a> {
        self.base
            .nontrivial_entries()
            .map(TensorProductFunction(IdentityFunction, &self.mapping_fns.0))
    }
}
