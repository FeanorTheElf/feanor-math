...
///  - Every ring `R` has a homomorphism `Z -> R`. The corresponding [`Homomorphism`]-object is
///    obtained with [`RingStore::int_hom()`], and the functionality provided by
///    [`RingBase::from_int()`].
///  - [`RingExtension`]s have give a (injective) homomorphism `R -> S` which can be obtained by
///    [`RingExtensionStore::inclusion()`]. The functionality is provided by functions on
///    [`RingExtension`], like [`RingExtension::from()`].
///  - Other "natural" homomorphisms can be obtained via [`RingStore::can_hom()`]. This requires the
///    underlying [`RingBase`]s to implement [`CanHomFrom`], and the functions of that trait define
///    the homomorphism.
///  
    ...
    Codomain: RingBase,
{
    ...
    type DomainStore: RingStore<Ring = Domain>;
    /// The type of the [`RingStore`] used by this object to store the codomain ring.
    type CodomainStore: RingStore<Ring = Codomain>;
    ...
    /// Applies this homomorphism to the given element from the domain ring,
    /// resulting in an element in the codomain ring.
    fn map_ref(&self, x: &Domain::Element) -> Codomain::Element { self.map(x.clone()) }
    ...
    fn mul_ref_map(&self, lhs: &Codomain::Element, rhs: &Domain::Element) -> Codomain::Element {
        self.mul_ref_snd_map(lhs.clone(), rhs)
    }
    ...
    S: RingBase + ?Sized,
{
    ...
    /// Evaluates the homomorphism on `rhs`, taking it by reference, and multiplies the result to
    /// `lhs`.
    ...
    /// Fused-multiply-add. Computes `summand + lhs * rhs`, where `rhs` is mapped into the ring via
    /// the homomorphism.
    fn fma_map_in(
        &self,
        from: &S,
    ...
    S::Ring: CanHomFrom<R::Ring>,
{
    from: R,
    to: S,
    data: <S::Ring as CanHomFrom<R::Ring>>::Homomorphism,
}
    ...
    S::Ring: CanHomFrom<R::Ring>,
{
    ...
    pub fn raw_hom(&self) -> &<S::Ring as CanHomFrom<R::Ring>>::Homomorphism { &self.data }

    /// Returns the underlying [`CanHomFrom::Homomorphism`], consuming this object.
    pub fn into_raw_hom(self) -> <S::Ring as CanHomFrom<R::Ring>>::Homomorphism { self.data }

    #[stability::unstable(feature = "enable")]
    pub fn from_raw_parts(from: R, to: S, data: <S::Ring as CanHomFrom<R::Ring>>::Homomorphism) -> Self {
        Self { from, to, data }
    }
}
...
impl<R, S> Homomorphism<R::Ring, S::Ring> for CanHom<R, S>
where
    R: RingStore,
    S: RingStore,
    S::Ring: CanHomFrom<R::Ring>,
...
///
/// More concretely, this should only be used when you only have a reference to `<R as
/// CanHomFrom<S>>::Homomorphism`, but cannot refactor code to wrap that object in a [`CanHom`]
/// instead. The main situation where this occurs is when implementing [`CanHomFrom`], since a the
/// lifetime of [`CanHom`] is bound by the lifetime of the domain and codomain rings, but
/// `CanHomFrom::Type` does not allow this.
#[stability::unstable(feature = "enable")]
    ...
    S::Ring: CanHomFrom<R::Ring>,
{
    from: R,
    to: S,
    data: &'a <S::Ring as CanHomFrom<R::Ring>>::Homomorphism,
}
    ...
    S::Ring: CanHomFrom<R::Ring>,
{
    ...
    #[stability::unstable(feature = "enable")]
    pub fn from_raw_parts(from: R, to: S, data: &'a <S::Ring as CanHomFrom<R::Ring>>::Homomorphism) -> Self {
        Self { from, to, data }
    }
}
...
impl<'a, R, S> Homomorphism<R::Ring, S::Ring> for CanHomRef<'a, R, S>
where
    R: RingStore,
    S: RingStore,
    S::Ring: CanHomFrom<R::Ring>,
...
where
    R: RingStore,
    S: RingStore,
    S::Ring: CanIsoFromTo<R::Ring>,
{
    from: R,
    to: S,
    data: <S::Ring as CanIsoFromTo<R::Ring>>::Isomorphism,
}
    ...
    S::Ring: CanIsoFromTo<R::Ring>,
{
    ...
    pub fn raw_iso(&self) -> &<S::Ring as CanIsoFromTo<R::Ring>>::Isomorphism { &self.data }

    /// Returns the underlying [`CanIsoFromTo::Isomorphism`], consuming this object.
    pub fn into_raw_iso(self) -> <S::Ring as CanIsoFromTo<R::Ring>>::Isomorphism { self.data }
}

impl<R, S> Homomorphism<S::Ring, R::Ring> for CanIso<R, S>
where
    R: RingStore,
    S: RingStore,
    S::Ring: CanIsoFromTo<R::Ring>,
...
/// assert_el_eq!(
///     extension,
///     extension.from_terms([(8, 0), (1, 1)].into_iter()),
///     &f
/// );
/// ```
#[derive(Clone, Debug)]
...
impl<R> Homomorphism<StaticRingBase<i32>, R::Ring> for IntHom<R>
where
    R: RingStore,
{
    type CodomainStore = R;
...
impl<R: RingStore> Homomorphism<R::Ring, R::Ring> for Identity<R> {
    type CodomainStore = R;
    type DomainStore = R;
    ...
    fn mul_assign_map_through_hom<First: ?Sized + RingBase, H: Homomorphism<First, R::Ring>>(
        &self,
        lhs: &mut El<R>,
        rhs: First::Element,
        hom: H,
    ...
    fn mul_assign_ref_map_through_hom<First: ?Sized + RingBase, H: Homomorphism<First, R::Ring>>(
        &self,
        lhs: &mut El<R>,
        rhs: &First::Element,
        hom: H,
...
where
    F: Send + Sync + Copy + Fn(&R, &S, &El<R>) -> El<S>,
    R: Copy,
    El<R>: Copy,
    S: Copy,
    El<S>: Copy,
...
impl<R: RingStore, S: RingStore, F> Homomorphism<R::Ring, S::Ring> for LambdaHom<R, S, F>
where
    F: Send + Sync + Fn(&R, &S, &El<R>) -> El<S>,
{
    type CodomainStore = S;
    ...
    fn map(&self, x: El<R>) -> <S::Ring as RingBase>::Element { (self.f)(self.domain(), self.codomain(), &x) }

    fn map_ref(&self, x: &El<R>) -> <S::Ring as RingBase>::Element { (self.f)(self.domain(), self.codomain(), x) }
}
...
///
///     fn delegate_ref<'a>(&self, el: &'a i32) -> &'a i32 { el }
///
///     fn delegate_mut<'a>(&self, el: &'a mut i32) -> &'a mut i32 { el }
///
///     fn delegate(&self, el: i32) -> i32 { el }
///
...
///     }
///
///     fn rev_delegate(&self, el: i32) -> i32 { el }
/// }
///
            ...
            for b in &elements {
                {
                    ...
                    assert!(
                        ...
                        from.formatted_el(a),
                        from.formatted_el(b),
                        to.formatted_el(&sum_map),
                        to.formatted_el(&map_sum),
                        to.formatted_el(&map_a),
                        to.formatted_el(&map_b)
                    );
                }
                {
                    let map_a = hom.map_ref(a);
                    ...
                    assert!(
                        ...
                        from.formatted_el(a),
                        from.formatted_el(b),
                        to.formatted_el(&prod_map),
                        to.formatted_el(&map_prod),
                        to.formatted_el(&map_a),
                        to.formatted_el(&map_b)
                    );
                }
                {
                    let map_a = hom.map_ref(a);
                    let prod_map = hom.map(from.mul_ref(a, b));
                    let mut mul_assign = map_a.clone();
                    hom.mul_assign_ref_map(&mut mul_assign, b);
                    assert!(
                        to.eq_el(&mul_assign, &prod_map),
                        "mul_assign_ref_map() failed: hom({} * {}) = {} != {} = mul_map(hom({}), {})",
                        from.formatted_el(a),
                        from.formatted_el(b),
                        to.formatted_el(&prod_map),
                        to.formatted_el(&mul_assign),
                        from.formatted_el(a),
                        from.formatted_el(b)
                    );
                }
                {
                    ...
                    assert!(
                        to.eq_el(&actual, &expected),
                        "mul_assign_ref_map() failed: hom({} * {} + {}) = {} != {} = fma_map(hom({}), {}, hom({}))",
                        from.formatted_el(a),
                        from.formatted_el(b),
                        from.formatted_el(&c),
                        to.formatted_el(&expected),
                        to.formatted_el(&actual),
                        from.formatted_el(a),
                        from.formatted_el(b),
                        from.formatted_el(&c)
                    );
