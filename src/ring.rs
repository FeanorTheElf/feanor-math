...
///     format!(
///         "{}",
///         poly_ring.formatted_el_within(&f, EnvBindingStrength::Weakest)
///     )
/// );
/// assert_eq!(
///     "X + 1",
///     format!(
///         "{}",
///         poly_ring.formatted_el_within(&f, EnvBindingStrength::Sum)
///     )
/// );
/// assert_eq!(
///     "(X + 1)",
///     format!(
///         "{}",
///         poly_ring.formatted_el_within(&f, EnvBindingStrength::Product)
///     )
/// );
/// assert_eq!(
///     "(X + 1)",
///     format!(
///         "{}",
///         poly_ring.formatted_el_within(&f, EnvBindingStrength::Power)
///     )
/// );
/// assert_eq!(
///     "(X + 1)",
///     format!(
///         "{}",
///         poly_ring.formatted_el_within(&f, EnvBindingStrength::Strongest)
///     )
/// );
/// ```
...
pub trait RingBase: PartialEq + Debug + Send + Sync {
    ...
    fn mul_int_ref(&self, lhs: &Self::Element, rhs: i32) -> Self::Element { self.mul_int(lhs.clone(), rhs) }
    ...
    /// assert_eq!(
    ///     Some(i16::MAX),
    ///     Zn64B::new(i16::MAX as u64).characteristic(&ZZ)
    /// );
    /// ```
...
macro_rules! delegate {
    ($base_trait:ty, fn $name:ident (&self, $($pname:ident: $ptype:ty),*) -> $rtype:ty) => {
        ...
        fn $name (&self, $($pname: $ptype),*) -> $rtype {
            <Self::Ring as $base_trait>::$name(self.get_ring(), $($pname),*)
        }
    };
    ($base_trait:ty, fn $name:ident (&self) -> $rtype:ty) => {
        #[doc = concat!(" See [`", stringify!($base_trait), "::", stringify!($name), "()`]")]
        fn $name (&self) -> $rtype {
            <Self::Ring as $base_trait>::$name(self.get_ring())
        }
    };
}
        ...
        match (&$ring, &$lhs, &$rhs) {
            (ring_val, lhs_val, rhs_val) => {
                assert!(
                    ...
                    "Assertion failed: {} != {}",
                    <_ as $crate::ring::RingStore>::formatted_el(ring_val, lhs_val),
...
pub trait RingStore: Sized + Send + Sync + Clone {
    ...
    type Ring: RingBase + ?Sized;

    /// Returns a reference to the stored ring.
    fn get_ring<'a>(&'a self) -> &'a Self::Ring;
    ...
    /// ```
    fn formatted_el<'a>(&'a self, value: &'a El<Self>) -> RingElementDisplayWrapper<'a, Self::Ring> {
    ...
    /// assert_eq!(
    ///     "X",
    ///     format!("{}", ring.formatted_el_within(&f, EnvBindingStrength::Sum))
    /// );
    /// assert_eq!(
    ///     "X",
    ///     format!(
    ///         "{}",
    ///         ring.formatted_el_within(&f, EnvBindingStrength::Product)
    ///     )
    /// );
    /// assert_eq!(
    ///     "X + 1",
    ///     format!("{}", ring.formatted_el_within(&g, EnvBindingStrength::Sum))
    /// );
    /// assert_eq!(
    ///     "(X + 1)",
    ///     format!(
    ///         "{}",
    ///         ring.formatted_el_within(&g, EnvBindingStrength::Product)
    ///     )
    /// );
    /// ```
    fn formatted_el_within<'a>(
        &'a self,
        value: &'a El<Self>,
        within: EnvBindingStrength,
    ) -> RingElementDisplayWrapper<'a, Self::Ring> {
...
pub trait RingExtension: RingBase {
    ...
    fn from_ref(&self, x: &El<Self::BaseRing>) -> Self::Element { self.from(x.clone()) }
    ...
    #[stability::unstable(feature = "enable")]
    fn mul_assign_base_through_hom<S: ?Sized + RingBase, H: Homomorphism<S, <Self::BaseRing as RingStore>::Ring>>(
        &self,
...
where
    S::Target: RingStore,
{
    type Ring = <<S as Deref>::Target as RingStore>::Ring;

    fn get_ring<'b>(&'b self) -> &'b Self::Ring { (**self).get_ring() }
}
...
impl<R: ?Sized + RingBase> RingStore for NeverRing<R> {
    ...
    fn get_ring<'a>(&'a self) -> &'a Self::Ring { self.0 }
}
...
pub mod generic_tests {
        ...
        S::Ring: CanIsoFromTo<R::Ring>,
    {
        ...
        for a in &elements {
            ...
            let map_in_out = to.get_ring().map_out(from.get_ring(), map_in.clone(), &iso);
            assert!(
                from.eq_el(&map_in_out, &a),
                "Bijectivity failed: {} != {} = hom^-1({}) = hom^-1(hom({}))",
                from.formatted_el(a),
                from.formatted_el(&map_in_out),
                to.formatted_el(&map_in),
                from.formatted_el(a)
            );
        }
    }
        ...
        R::Ring: SelfIso,
    {
        ...
        test_hom_axioms(&ring, &ring, elements.iter().map(|x| x.clone()));
        test_iso_axioms(&ring, &ring, elements.iter().map(|x| x.clone()));
    ...
    pub fn test_ring_axioms<R: RingStore, I: Iterator<Item = El<R>>>(ring: R, edge_case_elements: I) {
        ...
        for a in &elements {
            let a_minus_a = ring.sub(a.clone(), a.clone());
            assert!(
                ring.eq_el(&zero, &a_minus_a),
                "Additive inverse failed: {} - {} = {} != {}",
                ring.formatted_el_within(a, EnvBindingStrength::Sum),
                ring.formatted_el_within(a, EnvBindingStrength::Product),
                ring.formatted_el(&a_minus_a),
                ring.formatted_el(&zero)
            );
        }

        // check identity elements
        for a in &elements {
            let a_plus_zero = ring.add(a.clone(), zero.clone());
            assert!(
                ring.eq_el(a, &a_plus_zero),
                "Additive neutral element failed: {} + {} = {} != {}",
                ring.formatted_el_within(a, EnvBindingStrength::Sum),
                ring.formatted_el(&zero),
                ring.formatted_el(&a_plus_zero),
                ring.formatted_el(a)
            );

            let a_times_one = ring.mul(a.clone(), one.clone());
            assert!(
                ring.eq_el(a, &a_times_one),
                "Multiplicative neutral element failed: {} * {} = {} != {}",
                ring.formatted_el_within(a, EnvBindingStrength::Product),
                ring.formatted_el(&one),
                ring.formatted_el(&a_times_one),
                ring.formatted_el(a)
            );
        }
            ...
            for b in &elements {
                {
                    ...
                    assert!(
                        ...
                        ring.formatted_el_within(a, EnvBindingStrength::Sum),
                        ring.formatted_el_within(b, EnvBindingStrength::Sum),
                        ring.formatted_el(&ab),
                        ring.formatted_el(&ba),
                        ring.formatted_el_within(b, EnvBindingStrength::Sum),
                        ring.formatted_el_within(a, EnvBindingStrength::Sum)
                    );
                }
                ...
                if ring.is_commutative() {
                    ...
                    assert!(
                        ...
                        ring.formatted_el_within(a, EnvBindingStrength::Product),
                        ring.formatted_el_within(b, EnvBindingStrength::Product),
                        ring.formatted_el(&ab),
                        ring.formatted_el(&ba),
                        ring.formatted_el_within(b, EnvBindingStrength::Product),
                        ring.formatted_el_within(a, EnvBindingStrength::Product)
                ...
                for c in &elements {
                    {
                        ...
                        assert!(
                            ...
                            ring.formatted_el_within(a, EnvBindingStrength::Sum),
                            ring.formatted_el_within(b, EnvBindingStrength::Sum),
                            ring.formatted_el_within(c, EnvBindingStrength::Sum),
                            ring.formatted_el(&ab_c),
                            ring.formatted_el(&a_bc),
                            ring.formatted_el_within(a, EnvBindingStrength::Sum),
                            ring.formatted_el_within(b, EnvBindingStrength::Sum),
                            ring.formatted_el_within(c, EnvBindingStrength::Sum)
                        );
            ...
            for b in &elements {
                for c in &elements {
                    ...
                    assert!(
                        ...
                        ring.formatted_el_within(c, EnvBindingStrength::Sum),
                        ring.formatted_el(&ab_c),
                        ring.formatted_el(&ac_bc),
                        ring.formatted_el_within(a, EnvBindingStrength::Product),
                        ring.formatted_el_within(c, EnvBindingStrength::Product),
                        ring.formatted_el_within(b, EnvBindingStrength::Product),
                        ring.formatted_el_within(c, EnvBindingStrength::Product)
                    );
        ...
        for a in &elements {
            for b in &elements {
                for c in &elements {
                    let actual = ring.fma(a, b, c.clone());
                    let expected = ring.add(ring.mul_ref(a, b), c.clone());
                    assert!(
                        ...
                        ring.formatted_el(b),
                        ring.formatted_el(c),
                        ring.formatted_el(&actual),
                        ring.formatted_el(&expected),
                        ring.formatted_el_within(a, EnvBindingStrength::Product),
                        ring.formatted_el_within(b, EnvBindingStrength::Product),
                        ring.formatted_el_within(c, EnvBindingStrength::Sum)
                    );
                ...
                let actual = ring.get_ring().fma_int(a, 2, b.clone());
                let expected = ring.get_ring().add(ring.get_ring().add_ref(a, a), b.clone());
                assert!(
                    ring.eq_el(&expected, &actual),
        ...
        for a in &elements {
            for n in [0, 1, 2, 3, 7, 8] {
                ...
                assert!(
                    ...
                    ring.formatted_el(&expected),
                    ring.formatted_el(&actual),
                    ring.formatted_el_within(a, EnvBindingStrength::Power),
                    n
                );
            }
        }
