...
pub fn fincke_pohst<I, R, H, V, F>(
    h: H,
    quadratic_form: Submatrix<V, R::Element>,
    target: &[R::Element],
    radius_sqr: R::Element,
    mut for_point: F,
) where
    I: ?Sized + IntegerRing,
    ...
    F: FnMut(&[I::Element]) -> Option<R::Element>,
{
    ...
    // the inner product of the `i`-th column of `quadratic_form` with `(current[..i] - target[..i],
    // 0)`
    ...
    'visit_enum_tree_node: loop {
        ...
        for _ in 0..2 {
            let x = RR.sub_ref_snd(h.map_ref(&current[i]), &target[i]);
            ...
            if RR.is_lt(&norm_square, &radius_square) {
                ...
                if i == n {
                    ...
                    linear_components[i] = <_ as ComputeInnerProduct>::inner_product_ref_fst(
                        RR.get_ring(),
                        (0..i).map(|j| {
                            (
                                quadratic_form.at(j, i),
                                RR.sub_ref_snd(h.map_ref(&current[j]), &target[j]),
                            )
                        }),
                    );
