...
impl<R: ?Sized + RingBase> ComputeInnerProduct for R {
    default fn inner_product_ref_fst<'a, I: IntoIterator<Item = (&'a Self::Element, Self::Element)>>(
        &self,
        els: I,
    ) -> Self::Element
    where
    ...
    default fn inner_product_ref<'a, I: IntoIterator<Item = (&'a Self::Element, &'a Self::Element)>>(
        &self,
        els: I,
    ) -> Self::Element
    where
...
pub trait MatmulAlgorithm<R: ?Sized + RingBase> {
    ...
    ///
    /// This requires that `lhs` is a `nxk` matrix, `rhs` is a `kxm` matrix and `dst` is a `nxm`
    /// matrix. In this case, the function concretely computes `dst[i, j] += sum_l lhs[i, l] *
    /// rhs[l, j]` where `l` runs from `0` to `k - 1`.
...
pub trait StrassenHint: RingBase {
    ...
    ///
    /// The value is `0`, but if the considered rings have fast multiplication (compared to
    /// addition), then setting this higher may result in a performance gain.
...
impl<R: ?Sized + RingBase, A: Allocator> MatmulAlgorithm<R> for StrassenAlgorithm<A> {
        ...
        S: RingStore<Type = R> + Copy,
    {
        strassen::<_, _, _, _, _, T1, T2, T3>(
            true,
            <_ as StrassenHint>::strassen_threshold(ring.get_ring()),
            lhs,
            rhs,
            dst,
            ring,
...
/// true`, using the standard cubic formula for matrix multiplication.
///
    ...
    V3: AsPointerToSlice<El<R>>,
{
    ...
    for i in 0..lhs.row_count() {
        for j in 0..rhs.col_count() {
            let inner_prod = <_ as ComputeInnerProduct>::inner_product_ref(
                ring.get_ring(),
                (0..lhs.col_count()).map(|k| (lhs.at(i, k), rhs.at(k, j))),
            );
            if ADD_ASSIGN {
...
fn bench_schoolbook_matmul(bencher: &mut test::Bencher) {
    ...
    let lhs = OwnedMatrix::from_fn_in(
        BENCH_SIZE,
        BENCH_SIZE,
        |i, j| std::hint::black_box(i as BenchInt + j as BenchInt),
        Global,
    );
    let rhs = OwnedMatrix::from_fn_in(
        BENCH_SIZE,
        BENCH_SIZE,
        |i, j| std::hint::black_box(i as BenchInt + j as BenchInt),
        Global,
