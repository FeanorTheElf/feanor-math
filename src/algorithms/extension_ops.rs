    ...
    H: Homomorphism<<R::BaseRing as RingStore>::Type, <<P::Type as RingExtension>::BaseRing as RingStore>::Type>,
{
    ...
    _ = bisect_floor(StaticRing::<i64>::RING, 1, ring.rank() as i64, |d| {
        ...
        let sol_poly = |sol: &OwnedMatrix<El<<P::Type as RingExtension>::BaseRing>>| {
...
pub fn create_multiplication_matrix<R: RingStore, A: Allocator>(
    ...
    el: &El<R>,
    allocator: A,
) -> OwnedMatrix<El<<R::Type as RingExtension>::BaseRing>, A>
where
    R::Type: FreeAlgebra,
{
...
fn test_charpoly() {
    ...
    assert_el_eq!(
        ...
        charpoly(
            ring.get_ring(),
            &ring.add(ring.canonical_gen(), ring.pow(ring.canonical_gen(), 2)),
            &poly_ring,
            &ring.base_ring().identity()
        )
    );
...
fn test_minpoly() {
    ...
    assert_el_eq!(
        ...
        minpoly(
            ring.get_ring(),
            &ring.add(ring.one(), ring.pow(ring.canonical_gen(), 3)),
            &poly_ring,
            &ring.base_ring().identity()
        )
    );
