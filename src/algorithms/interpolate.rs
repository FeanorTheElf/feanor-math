...
/// the input elements are degree-1 polynomials, this becomes `O(n sum_(0 <= i <= log n) 2^(i c)) =
/// O(n^(c + 1))` where `c` is the multiplication exponent, e.g. `c = 1.58...` for Karatsuba
/// multiplication. Unfortunately, this means that a single complete multiplication together with
/// `n` polynomial divisions by the monic linear factors is faster.
///
/// # Example
/// ```rust
...
/// Concretely, this is the univariate polynomial `f` of degree `< x.len()` such that `f(x[i]) =
/// y[i]` for all `i`.
///
/// If no such polynomial exists (this is only possible if the base ring is not a field), an
/// error is returned.
///
/// The complexity is `O(n T(n))` where `T(n)` is the cost of multiplying two degree-`n`
/// polynomials.
///
/// # Example
/// ```rust
...
#[stability::unstable(feature = "enable")]
#[instrument(skip_all, level = "trace")]
pub fn interpolate<P, V1, V2, A: Allocator>(
    poly_ring: P,
    x: V1,
    y: V2,
    allocator: A,
...
where
    ...
    V1: VectorFn<El<BaseRingStore<P>>>,
    V2: VectorFn<El<BaseRingStore<P>>>,
{
    ...
    let denoms_inv = denoms
        .iter()
        .map(|den| base_ring.invert(den).ok_or(()))
    ...
    if let Ok(denoms_inv) = denoms_inv {
        return Ok(poly_ring.from_terms((0..x.len()).map(|i| {
            (
                <_ as ComputeInnerProduct>::inner_product_ref_fst(
                    ...
                    (0..x.len()).map(|j| {
                        (
                            poly_ring.coefficient_at(&nums[j], i),
                            base_ring.mul_ref_snd(y.at(j), &denoms_inv[j]),
                        )
                    }),
                ),
                i,
        ...
        return poly_ring.try_from_terms(poly_ring.terms(&scaled_result).map(|(c, i)| {
            base_ring
                .checked_div(&c, &denominator)
                .map(|c| (c, i))
...
pub fn interpolate_multivariate<P, V1, V2, A, A2>(
    ...
    mut values: Vec<El<BaseRingStore<P>>, A2>,
    allocator: A,
) -> Result<El<P>, InterpolationError>
where
    P: RingStore,
    ...
    for i in (0..n).rev() {
        ...
        for outer_block_index in 0..outer_block_count {
            for inner_block_index in 0..leading_dim {
                ...
                for j in 0..len {
                    values[block_start + leading_dim * j] = uni_poly_ring.coefficient_at(&poly, j).clone();
                }
            }
        }
    }
...
fn test_interpolate() {
    ...
    let actual = interpolate(
        &poly_ring,
        x.clone(),
        x.map_fn(|x| poly_ring.evaluate(&poly, &x, &ring.identity())),
        Global,
    )
    .unwrap();
    ...
    for i in 0..4 {
        assert_el_eq!(
            ring,
            ring.zero(),
            poly_ring.evaluate(&actual, &ring.from_fraction(i, 1), ring.identity())
        );
    }
        ...
        for y in 0..3 {
            for z in 0..4 {
                ...
                assert_el_eq!(
                    ...
                    poly_ring.evaluate(
                        &poly,
                        [ring.int_hom().map(x), ring.int_hom().map(y), ring.int_hom().map(z)].into_clone_els(),
                        &ring.identity()
                    )
                );
            }
