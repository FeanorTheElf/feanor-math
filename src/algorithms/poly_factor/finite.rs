        ...
        <BaseRing<P> as RingStore>::Type: FiniteRing + Field + SelfIso
{
    ...
    event!(Level::TRACE, poly_deg = poly_ring.degree(f).unwrap(), field_size_bits = BigIntRing::RING.abs_log2_ceil(&poly_ring.base_ring().size(BigIntRing::RING).unwrap()).unwrap());
