        ...
        <<P::Type as RingExtension>::BaseRing as RingStore>::Type: FiniteRing + Field + SelfIso
{
    ...
    span!(Level::INFO, "factor_poly_GF", poly_deg = poly_ring.degree(f).unwrap(), field_size = %BigIntRing::RING.formatted_el(&poly_ring.base_ring().size(BigIntRing::RING).unwrap())).in_scope(|| {
