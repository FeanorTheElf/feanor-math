...
pub fn poly_factor_extfield_squarefree<P>(
    LX: P,
    f: &El<P>,
    attempts: usize,
        ...
        PerfectField + PolyTFracGCDRing + FactorPolyField + InterpolationBaseRing + FiniteRingSpecializable + SelfIso,
{
    ...
    // `L`; there are only finitely many `k` that don't work, by the following idea:
    // Write `f = f1 ... fr` for the factorization of `f`, where `r <= d`. Then `N(f(X + k theta))` is
    // not square-free, iff there exist `i != j` and `sigma` such that `fi(X + k theta) = sigma(fj(X
    // + k theta)) = sigma(fj)(X + k sigma(theta))`. Now note that for given `i, j, sigma`, `fi(X +
    // k theta) = sigma(fj)(X + k sigma(theta))` for at most one `k`, as otherwise `fi(a (k1 - k2)
    // theta) sigma(theta)) = sigma(fj)(a (k1 - k2) sigma(theta))` for all `a in Z` (evaluation is ring
    // hom).
    //
    // Now note that `fi(X) - sigma(fj(X))` is a linear combination of `X^m` and `sigma(X^m)`, i.e. of
    // `deg(fi) + deg(fj) <= d` basic functions. The vectors `(a1 theta)^m, ..., (al theta)^m` for
    // `m <= deg(fi)` and `sigma(a1 theta)^m, ..., sigma(al theta)^m` for `m <= deg(fj)` are all
    // linearly independent (assuming `a1, ..., al` distinct and `l >= deg(fi) + deg(fj) + 2`), thus
    // `char(K) >= d + 2` (or `char(K) = 0`) implies that `fi = fj = 0`, a contradiction.
    //
    // Thus we find that there are at most `d(d + 1)/2 * [L : K]` many `k` such that `N(f(X + k theta))`
