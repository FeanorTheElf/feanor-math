...
fn test_solve() {
    ...
    let ring = FreeAlgebraImpl::new(base_ring, [14, 0, 14]);
    ...
    STANDARD_MATMUL.matmul(TransposableSubmatrix::from(A.data()), TransposableSubmatrix::from(sol.data()), TransposableSubmatrixMut::from(prod.data_mut()), ring.get_ring());
