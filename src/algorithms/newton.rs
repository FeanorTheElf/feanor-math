    ...
    <P::Type as RingExtension>::BaseRing: RingStore<Type = Complex64Base>,
{
    ...
    // we bound `|f'(x + t) - f'(x)| <= epsilon` for `t` at most `assume_radius` using the taylor
    // series
    ...
    // The idea is as follows: We have `f(x + t) = f(x) + f'(g(t)) t` where `g(t)` is a value
    // between `x` and `x + t`; Using `|f'(g(t))| >= |f'(x)| - f_prime_bound` and rearranging it
    // gives `t <= |f(x)| / (|f'(x)| - f_prime_bound)`; For this to be at most `assume_radius`,
...
fn test_find_all_approximate_complex_roots() {
    ...
    let expected = (1..=52)
        .rev()
        .filter(|i| signed_gcd(*i, 105, StaticRing::<i64>::RING) == 1)
