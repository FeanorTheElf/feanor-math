    ...
    where F: Fn(T, T) -> T, T: Clone + Hash + Eq + std::fmt::Debug
{
    let pow = |x: &T, e: i64| {
        debug_assert!(e >= 0);
        algorithms::sqr_mul::generic_pow_shortest_chain_table(x.clone(), &e, ZZ, |a| Ok(op(a.clone(), a.clone())), |a, b| Ok(op(a.clone(), b.clone())), |a| a.clone(), identity.clone()).unwrap_or_else(|e| e)
    };
    assert!(pow(&base, order) == identity);
    let factorization = algorithms::int_factor::factor(&ZZ, order);
    for (p, _) in &factorization {
        assert!(pow(&base, order / p) != identity);
    }
