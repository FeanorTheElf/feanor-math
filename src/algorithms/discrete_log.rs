...
impl<G: AbelianGroupStore> SubgroupBase<G> {
    ...
    pub fn subgroup_order(&self) -> El<BigIntRing> {
        ...
        for i in 0..self.order_factorization.len() {
            ...
            let Zpne = zn_big::Zn::new(
                ZZbig,
                ZZbig.pow(int_cast(p, ZZbig, StaticRing::<i64>::RING), e * n),
            );
            let mod_pne = Zpne.can_hom(&StaticRing::<i64>::RING).unwrap();
            let relation_lattice_det = determinant_using_pre_smith(
                &Zpne,
                OwnedMatrix::from_fn(
                    relation_lattice.row_count(),
                    relation_lattice.col_count(),
                    |k, l| mod_pne.map(*relation_lattice.at(k, l)),
                )
                .data_mut(),
                Global,
            );
    ...
    pub fn add_generator(self, new_gen_base: GroupEl<G>) -> Self {
        ...
        for p_idx in 0..self.order_factorization.len() {
            ...
            for i in 0..n {
                for j in 0..n {
                    *main_relation_matrix.at_mut(i, j) =
                        *self.scaled_relation_lattices[p_idx][e].at(i, j);
                }
            }
            *main_relation_matrix.at_mut(n, n) = -ZZ.pow(p, e);
            for k in 0..e {
                if let Some(dlog) = self.padic_dlog(
                    p_idx,
                    e,
                    &group.pow(&new_gen, &ZZbig.pow(ZZbig.clone_el(&p_bigint), k)),
                ) {
            ...
            debug_assert!(
                main_relation_matrix
                    .data()
                    .row_iter()
                    .all(|row| group.is_identity(&(0..n).fold(
                        group.pow(&new_gen, &ZZ_to_ZZbig.map(row[n])),
                        |current, i| {
                    ...
                    .map(|row| {
                        ...
                        let result = (0..n).fold(
                            ...
                            |current, j| {
                                group.op(
                                    current,
                                    group.pow(&gens[j], &ZZ_to_ZZbig.mul_ref_map(&scale, &row[j])),
                                )
                            },
                        );
    ...
    fn padic_dlog(&self, p_idx: usize, e: usize, target: &GroupEl<G>) -> Option<Vec<i64>> {
        ...
        // here we use the power-of-`p` map and the fact that `G/H ~ pG` to compute the dlog in
        // `G/H`
        let G_mod_H_dlog =
            self.padic_dlog(p_idx, e - 1, &group.pow(target, &ZZ_to_ZZbig.map(p)))?;
        debug_assert!(group.eq_el(
            &group.pow(target, &ZZ_to_ZZbig.map(p)),
            &(0..n).fold(group.identity(), |current, i| group.op(
                current,
                group.pow(&gens[i], &ZZ_to_ZZbig.map(p * G_mod_H_dlog[i]))
            ))
        ));
        ...
        let delta = (0..n).fold(group.clone_el(target), |current, i| {
            group.op(
                current,
                group.pow(&gens[i], &ZZ_to_ZZbig.map(-G_mod_H_dlog[i])),
            )
        });
    ...
    fn padic_rectangular_form<'a>(&'a self, p_idx: usize) -> Vec<(GroupEl<G>, usize)> {
        ...
        let mut relation_lattice_mod_pne = OwnedMatrix::from_fn(
            relation_lattice.row_count(),
            relation_lattice.col_count(),
            |k, l| mod_pne.map(*relation_lattice.at(k, l)),
        );
        ...
        impl<'a, G: AbelianGroupStore> TransformTarget<zn_big::ZnBase<BigIntRing>>
            for TransformGenerators<'a, G>
        {
            fn transform<S: Copy + RingStore<Type = zn_big::ZnBase<BigIntRing>>>(
                ...
                let inv_transform = [
                    ...
                    ring.smallest_positive_lift(
                        ring.negate(ring.mul_ref(&transform[1], &transform_inv_det)),
                    ),
                    ring.smallest_positive_lift(
                        ring.negate(ring.mul_ref(&transform[2], &transform_inv_det)),
                    ),
            ...
            .map(|(i, g)| {
                (
                    ...
                    int_cast(
                        ZZbig.ideal_gen(
                            Zpne.modulus(),
                            &Zpne.smallest_positive_lift(
                                Zpne.clone_el(relation_lattice_mod_pne.at(i, i)),
                            ),
                        ),
                        ZZ,
                        ZZbig,
    ...
    G::Type: SerializableElementGroup,
{
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        DeserializeSeedNewtypeStruct::new(
            "Subgroup",
            DeserializeSeedDependentTuple::new(PhantomData::<G>, |group| {
                DeserializeSeedSubgroupData { group }
            }),
        )
    ...
    fn clone(&self) -> Self {
        Self {
            ...
            scaled_relation_lattices: self
                ...
                .map(|x| {
                    x.iter()
                        .map(|x| x.clone_matrix(StaticRing::<i64>::RING))
                        .collect()
                })
                .collect(),
        }
...
/// assert_eq!(
///     Some(vec![1]),
///     baby_giant_step(&group, 0, &[0], &[BigIntRing::RING.power_of_two(10)])
/// );
/// ```
///
...
///     baby_giant_step(
///         &group,
///         group.pow(&x, &int_cast(3, BigIntRing::RING, StaticRing::<i64>::RING)),
///         &[x],
///         &[BigIntRing::RING.power_of_two(4)]
///     )
/// );
/// ```
#[stability::unstable(feature = "enable")]
    ...
    G: AbelianGroupStore,
{
    ...
    let ns = dlog_bounds
        ...
        .collect::<Vec<_>>();
    let count = int_cast(
        ZZbig.prod(ns.iter().map(|n| int_cast(*n, ZZbig, ZZ))),
        ZZ,
        ZZbig,
    );
    let mut baby_step_table: HashMap<HashableGroupEl<_>, i64> =
    ...
    // fill baby step table
    {
        ...
        for idx in 0..count {
            _ = baby_step_table.insert(
                HashableGroupEl::new(&group, group.clone_el(&current_els[n - 1])),
                idx,
            );
            ...
            current_idxs[i] += 1;
            current_els[i] = group.op_ref_snd(
                replace(&mut current_els[i], group.identity()),
                &generators[i],
            );
            for j in (i + 1)..n {
    ...
    {
        let start_el = giant_steps
            .iter()
            .fold(group.identity(), |x, y| group.op_ref_snd(x, y));
        let mut current_els = (0..n)
            .map(|_| group.clone_el(&start_el))
            .collect::<Vec<_>>();
        let mut current_idxs = (0..n).map(|_| 1).collect::<Vec<_>>();
        for idx in 0..count {
            if let Some(bs_idx) = baby_step_table.get(&HashableGroupEl::new(
                &group,
                group.clone_el(&current_els[n - 1]),
            )) {
                ...
                if (0..dlog_bounds.len())
                    .all(|j| ZZbig.is_leq(&int_cast(result[j], ZZbig, ZZ), &dlog_bounds[j]))
                {
...
fn test_baby_giant_step() {
    ...
    assert_eq!(
        ...
        baby_giant_step(
            ...
            85,
            &[10, 1],
            &from_fn::<_, 2, _>(|_| int_cast(8, ZZbig, ZZ))
        )
    );
    ...
    assert_eq!(
        ...
        baby_giant_step(
            ...
            85,
            &[10, 1],
            &from_fn::<_, 2, _>(|_| int_cast(21, ZZbig, ZZ))
        )
    );
    ...
    assert_eq!(
        ...
        baby_giant_step(
            ...
            85,
            &[10, 1],
            &from_fn::<_, 2, _>(|_| int_cast(31, ZZbig, ZZ))
        )
    );
}
...
fn test_padic_relation_lattice() {
    ...
    assert_matrix_eq!(
        ZZ,
        [[-81]],
        subgroup.get_group().scaled_relation_lattices[0][4]
    );
    assert_matrix_eq!(
        ZZ,
        [[-27]],
        subgroup.get_group().scaled_relation_lattices[0][3]
    );
    assert_matrix_eq!(
        ZZ,
        [[-9]],
        subgroup.get_group().scaled_relation_lattices[0][2]
    );
    assert_matrix_eq!(
        ZZ,
        [[-3]],
        subgroup.get_group().scaled_relation_lattices[0][1]
    );
    assert_matrix_eq!(
        ZZ,
        [[1]],
        subgroup.get_group().scaled_relation_lattices[0][0]
    );
...
fn random_test_dlog() {
    ...
    for _ in 0..RANDOM_TEST_INSTANCE_COUNT {
        ...
        let val = (0..3).fold(G.identity(), |current, i| {
            G.op(
                current,
                G.pow(&gs[i], &int_cast(coeffs[i] as i64, ZZbig, ZZ)),
            )
        });
...
fn test_zn_dlog() {
    ...
    assert_eq!(
        0,
        finite_field_discrete_log(ring.one(), g1, ring).unwrap() % 16
    );
    assert_eq!(
        0,
        finite_field_discrete_log(ring.one(), g2, ring).unwrap() % 2
    );
