...
pub fn baby_giant_step<T, F, I>(value: T, base: &T, integer_ring: I, base_order: &El<I>, op: F, identity: T) -> Option<El<I>> 
    where F: Fn(T, T) -> T, T: Clone + Hash + Eq, I: IntegerRingStore
{
...
fn power_p_discrete_log<T, F, I>(value: T, p_e_base: &T, integer_ring: I, p: &El<I>, e: u32, op: F, identity: T) -> Option<El<I>> 
    where F: Fn(T, T) -> T, T: Clone + Hash + Eq + std::fmt::Debug, I: IntegerRingStore
{
    ...
    for i in 0..e {
        let log = baby_giant_step(pow(&current, &p.pow(e - i - 1)), &p_base, integer_ring, &p, &op, identity.clone())?;
        let p_i = p.pow(i);
        let fill = (p - log) * p_i;
    ...
    where F: Fn(T, T) -> T, T: Clone + Hash + Eq + std::fmt::Debug, I: IntegerRingStore
{
    ...
    for (p, e) in order.clone().factor() {
        ...
        (current_log, current_size) = crt(log, current_log, &size, &current_size, integer_ring);
    }
...
/// 
pub fn discrete_log_2d<T, F, I>(value: T, base: (&T, &T), order: (&El<I>, &El<I>), integer_ring: I, op: F, identity: T) -> Option<(El<I>, El<I>)> 
    where F: Fn(T, T) -> T, T: Clone + Hash + Eq + std::fmt::Debug, I: IntegerRingStore
{
    let pow = |x: &T, e: &El<I>| algorithms::sqr_mul::generic_abs_square_and_multiply(x, e, integer_ring, |a, b| op(a, b), |a, b| op(a.clone(), b.clone()), identity.clone());
    let mut current_log = (integer_ring.one(), integer_ring.ring().one());
    let mut current_size = (integer_ring.one(), integer_ring.one());
