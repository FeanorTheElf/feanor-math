    ...
    P::Ring: PolyRing,
{
    ...
    for i in 1..=StaticRing::<i64>::RING.abs_log2_ceil(&(n as i64)).unwrap() {
        let mut square = poly_ring.pow(current.clone(), 2);
...
#[instrument(skip_all, level = "trace")]
pub fn multipointeval<P>(poly_ring: P, poly: &El<P>, points: &[El<BaseRingStore<P>>]) -> Vec<El<BaseRingStore<P>>>
where
    P: RingStore,
    P::Ring: PolyRing,
{
    ...
    let mut current = vec![(
        deg_poly,
        poly_ring.from_terms(poly_ring.terms(poly).map(|(c, i)| (c.clone(), deg_poly - i))),
    )];
    while current.len() < points.len() {
        ...
        let new = polys
            ...
            .map(|(modulus, (deg_poly, poly))| {
                ...
                let mut remainder = poly_ring.from_terms(
                    (0..=deg_remainder)
                        .map(|i| (poly_ring.coefficient_at(&poly_div_modulus, i + deg_offset).clone(), i)),
                );
    ...
    return current
        ...
        .map(|(deg_rem, rem)| {
            assert!(deg_rem == 0);
            poly_ring.coefficient_at(&rem, 0).clone()
        })
        .collect();
}
...
fn test_multipointeval() {
    ...
    let FpX = DensePolyRing::new(Zn64B::new(65537), "X");
    let points = (0..99)
