...
where
    F: Fn(&mut T, &mut T, &mut T, &S, &S) + Clone,
{
    ...
    assert_eq!(
        twiddles.as_chunks::<2>().0.len(),
        data.chunks_mut(3 * stride).len()
    );

    if stride == 1 {
        for ([twiddle1, twiddle2], butterfly_data) in twiddles
            .as_chunks::<2>()
            .0
            .iter()
    ...
    A: Allocator,
{
        ...
        F: FnMut(i64) -> R_twiddle::Element,
    {
        ...
        return Self {
            ...
            inv_twiddles: Self::create_inv_twiddle_list(
                hom.domain(),
                log3_n,
                &mut root_of_unity_pow,
            ),
    ...
    pub fn change_ring<R_new: ?Sized + RingBase, H_new: Homomorphism<R_twiddle, R_new>>(
        self,
        new_hom: H_new,
    ) -> (CooleyTukeyRadix3FFT<R_new, R_twiddle, H_new, A>, H) {
        ...
        let root_of_unity = if self.log3_n == 0 {
            ...
            let root_of_unity =
                &self.inv_twiddles[self.log3_n - 1][threeadic_reverse(1, self.log3_n - 1)];
            debug_assert!(self.ring().eq_el(
                &self.hom.map_ref(root_of_unity),
                self.root_of_unity(self.hom.codomain())
            ));
    ...
    #[stability::unstable(feature = "enable")]
    pub fn with_allocator<A_new: Allocator>(
        self,
        allocator: A_new,
    ) -> CooleyTukeyRadix3FFT<R_main, R_twiddle, H, A_new> {
        CooleyTukeyRadix3FFT {
            twiddles: self.twiddles,
            inv_twiddles: self.inv_twiddles,
    ...
    fn create_twiddle_list<F>(
        ring: &H::DomainStore,
        log3_n: usize,
        mut pow_zeta: F,
    ) -> Vec<Vec<R_twiddle::Element>>
    where
        F: FnMut(i64) -> R_twiddle::Element,
    {
        let n = ZZ.pow(3, log3_n);
    ...
    fn butterfly_step_main<const INV: bool>(&self, data: &mut [R_main::Element], step: usize) {
        ...
        butterfly_loop(
            ...
            step,
            twiddles,
            |x, y, z, twiddle1, twiddle2| {
                if INV {
                    <R_main as CooleyTukeyRadix3Butterfly<R_twiddle>>::inv_butterfly(
                        &self.hom,
                        x,
                        y,
                        z,
                    ...
                    <R_main as CooleyTukeyRadix3Butterfly<R_twiddle>>::butterfly(
                        &self.hom,
                        x,
                        y,
                        z,
        ...
        // BUTTERFLY_RADIX3_TIMES[step].fetch_add((end - start).as_micros() as usize,
        // std::sync::atomic::Ordering::Relaxed);
    }

    fn fft_impl(&self, data: &mut [R_main::Element]) {
        for i in 0..data.len() {
            <R_main as CooleyTukeyRadix3Butterfly<R_twiddle>>::prepare_for_fft(
    ...
    fn inv_fft_impl(&self, data: &mut [R_main::Element]) {
        for i in 0..data.len() {
            <R_main as CooleyTukeyRadix3Butterfly<R_twiddle>>::prepare_for_inv_fft(
                self.hom.codomain().get_ring(),
                &mut data[i],
            );
        }
        ...
        let n_inv = self
            .hom
            .domain()
            .invert(&self.hom.domain().int_hom().map(self.len() as i32))
            .unwrap();
        for i in 0..data.len() {
    ...
    A: Allocator,
{
    ...
    {
        assert!(
            ring.get_ring() == self.hom.codomain().get_ring(),
            "unsupported ring"
        );
...
pub trait CooleyTukeyRadix3Butterfly<S: ?Sized + RingBase>: RingBase {
    /// Should compute `(a, b, c) := (a + t b + t^2 c, a + t z b + t^2 z^2 c, a + t z^2 b + t^2 z
    /// c)`.
    ///
    ...
    /// [`CooleyTukeyRadix3Butterfly::butterfly()`] or of
    /// [`CooleyTukeyRadix3Butterfly::prepare_for_fft()`].
    ...
    ///
    /// It is guaranteed that the input elements are either outputs of
    /// [`CooleyTukeyRadix3Butterfly::inv_butterfly()`] or of
    /// [`CooleyTukeyRadix3Butterfly::prepare_for_inv_fft()`].
    ...
    /// [`CooleyTukeyRadix3Butterfly::butterfly()`] that the inputs are in this form.
    #[inline(always)]
    ...
    /// bring ring element into a certain form, and assume during
    /// [`CooleyTukeyRadix3Butterfly::inv_butterfly()`] that the inputs are in this form.
    #[inline(always)]
...
impl<R: ?Sized + RingBase, S: ?Sized + RingBase> CooleyTukeyRadix3Butterfly<S> for R {
    ...
    /// Possibly pre-processes elements before the inverse FFT starts. Here you can bring ring
    /// element into a certain form, and assume during
    /// [`CooleyTukeyRadix3Butterfly::inv_butterfly()`] that the inputs are in this form.
    #[inline(always)]
    ...
    A: Allocator,
{
    fn expected_absolute_error(&self, input_bound: f64, input_error: f64) -> f64 {
        ...
        let multiply_absolute_error =
            2. * input_bound * root_of_unity_error() + input_bound * f64::EPSILON;
...
fn test_radix3_butterflies() {
    ...
    let fft = CooleyTukeyRadix3FFT::new_with_hom(
        ...
        ring_fastmul.coerce(&ring, zeta),
        log3_n,
    );

    const LEN: usize = 27;
    let data: [_; LEN] = from_fn(|j| i(j as i32));
    let expected_std_order = |step: usize, group_idx: usize, value_idx: usize| {
        ring.sum((0..ZZ.pow(3, step)).map(|k| {
            ring.mul(
                ring.pow(
                    zeta_inv,
                    value_idx * (k * ZZ.pow(3, log3_n - step)) as usize,
                ),
                data[group_idx + (k * ZZ.pow(3, log3_n - step)) as usize],
            )
        }))
    };
    let expected_threeadic_reverse = |step: usize| {
        from_fn(|i| {
            expected_std_order(
                step,
...
fn test_radix3_inv_fft() {
    ...
    let data = (0..ZZ.pow(3, log3_n))
        .map(|x| ring.int_hom().map(x as i32))
        .collect::<Vec<_>>();
...
fn test_size_1_fft() {
    let ring = Fp::<17>::RING;
    let fft = CooleyTukeyRadix3FFT::for_zn(&ring, 0)
        .unwrap()
...
pub mod generic_tests {
    ...
    pub fn test_cooley_tuckey_radix3_butterfly<
        R: RingStore,
        S: RingStore,
            ...
            for b in &elements {
                for c in &elements {
                    let [mut x, mut y, mut z] =
                        [ring.clone_el(a), ring.clone_el(b), ring.clone_el(c)];
                    <R::Type as CooleyTukeyRadix3Butterfly<S::Type>>::prepare_for_fft(
                        ring.get_ring(),
                        &mut x,
                    );
                    <R::Type as CooleyTukeyRadix3Butterfly<S::Type>>::prepare_for_fft(
                    ...
                    assert_el_eq!(
                        ...
                        ring.add_ref_fst(
                            a,
                            ring.mul_ref_snd(ring.add_ref_fst(b, ring.mul_ref(c, &t)), &t)
                        ),
                        &x
                    );
                    ...
                    <R::Type as CooleyTukeyRadix3Butterfly<S::Type>>::prepare_for_inv_fft(
                        ring.get_ring(),
                        &mut x,
                    );
                    <R::Type as CooleyTukeyRadix3Butterfly<S::Type>>::prepare_for_inv_fft(
