        ...
        LinSolveRing + SelfIso + ZnRing + FromModulusCreateableZnRing + Clone,
{
    ...
    let (inverse, _) = local_zn_ring_bezout_identity(
        &poly_ring,
        &poly_ring.add_ref_fst(&modulus, poly),
        &modulus,
    )?;
    return Some(
        ring.from_canonical_basis_extended(
            (0..=poly_ring.degree(&inverse).unwrap())
                .map(|i| base_ring.clone_el(poly_ring.coefficient_at(&inverse, i))),
        ),
    );
}
...
fn test_invert_over_local_zn() {
    ...
    let array =
        |data: [i32; 4]| std::array::from_fn::<_, 4, _>(|i| base_ring.int_hom().map(data[i]));
    ...
    assert_el_eq!(
        ...
        ring.one(),
        ring.mul(
            invert_over_local_zn(RingRef::new(ring.get_ring()), &a).unwrap(),
            a
        )
    );
    ...
    let array =
        |data: [i32; 2]| std::array::from_fn::<_, 2, _>(|i| base_ring.int_hom().map(data[i]));
