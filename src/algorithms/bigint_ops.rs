...
fn core_fma_small_ref_fst<A: Allocator>(lhs: &[BlockInt], factor: BlockInt, summand: &mut Vec<u64, A>, shift: u32) {
    ...
    let mut implementation = || {
        ...
        let lhs_shifted_block = |i: usize| {
            (*lhs.get(i.wrapping_sub(block_offset)).unwrap_or(&0) << remaining_shift)
                | lhs
                    .get(i.wrapping_sub(block_offset + 1))
                    .unwrap_or(&0)
                    .checked_shr(BLOCK_BITS - remaining_shift)
                    .unwrap_or(0)
        };
...
pub fn core_fma_small_ref_snd<A: Allocator>(lhs: &mut Vec<u64, A>, factor: BlockInt, summand: &[u64], shift: u32) {
    ...
    let mut implementation = || {
        ...
        for i in block_offset..(up_to + 1) {
            let prod = (((lhs[i] as u64) << remaining_shift) + shift_buffer) as u128 * factor as u128
                + buffer as u128
                + *summand.get(i).unwrap_or(&0) as u128;
...
/// This function does not truncate the resulting vector.
#[inline(always)]
fn core_fms_small_ref_fst<A: Allocator>(
    rhs: &[BlockInt],
    factor: BlockInt,
    summand: &mut Vec<u64, A>,
    shift: u32,
) -> Result<(), ()> {
    let block_offset = (shift / BLOCK_BITS) as usize;
    let remaining_shift = shift % BLOCK_BITS;
    ...
    let mut implementation = || {
        ...
        let rhs_shifted_block = |i: usize| {
            (*rhs.get(i.wrapping_sub(block_offset)).unwrap_or(&0) << remaining_shift)
                | rhs
                    .get(i.wrapping_sub(block_offset + 1))
                    .unwrap_or(&0)
                    .checked_shr(BLOCK_BITS - remaining_shift)
        ...
        let up_to = max(
            effective_length(rhs.as_ref()) + block_offset + 1,
            effective_length(summand),
        );
        summand.truncate(up_to + 1);
        expand(summand, up_to + 1);
        let mut buffer: u64 = 0;
        for i in block_offset..(up_to + 1) {
            let value =
                (summand[i] as u128).wrapping_sub(buffer as u128 + rhs_shifted_block(i) as u128 * factor as u128);
...
pub fn bigint_cmp_small(lhs: &[BlockInt], rhs: DoubleBlockInt) -> Ordering {
    bigint_cmp(
        lhs,
        &[
            (rhs & ((1 << BLOCK_BITS) - 1)) as BlockInt,
            (rhs >> BLOCK_BITS) as BlockInt,
        ],
    )
}
...
#[stability::unstable(feature = "enable")]
#[instrument(skip_all, level = "trace")]
pub fn bigint_fma_small_ref_fst<A: Allocator>(
    lhs: &[BlockInt],
    factor: BlockInt,
    summand: &mut Vec<u64, A>,
    shift: u32,
...
#[stability::unstable(feature = "enable")]
#[instrument(skip_all, level = "trace")]
pub fn bigint_fms_small_ref_fst<A: Allocator>(
    rhs: &[BlockInt],
    factor: BlockInt,
    summand: &mut Vec<u64, A>,
    shift: u32,
...
pub fn bigint_rshift<A: Allocator>(lhs: &mut Vec<BlockInt, A>, power: usize) {
    ...
    for i in 0..(lhs_len - block_offset) {
        lhs[i] = (lhs[i + block_offset] >> remaining_shift)
            | lhs
                .get(i + block_offset + 1)
                .unwrap_or(&0)
                .checked_shl(BLOCK_BITS - remaining_shift)
                .unwrap_or(0)
    }
...
pub fn bigint_fma<A: Allocator>(lhs: &[BlockInt], rhs: &[BlockInt], mut summand: Vec<BlockInt, A>) -> Vec<BlockInt, A> {
    summand.resize(
        max(
            effective_length(lhs) + effective_length(rhs),
            effective_length(summand.as_ref()),
        ),
        0,
    );
...
fn test_core_fma_small() {
    ...
    assert_eq!(
        parse("3873816255479005839460000000000000000000000000000000", 10),
        truncate_zeros(z)
    );
