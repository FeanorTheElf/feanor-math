    ...
    R: ?Sized + ApproxRealField,
{
    ...
    let pivot_inv_error = if RR.is_geq(den_err, den) {
        ...
        RR.div(den_err, &RR.mul_ref_fst(den, RR.sub_ref(den, den_err)))
    };
    ...
    V3: AsPointerToSlice<R::Element>,
{
    ...
    for k in 0..i {
        let sum = RR.sub(
            ...
            RR.sum((0..k).map(|l| RR.mul_ref(gso.cholesky.at(l, i), gso.cholesky.at(l, k)))),
        );
        ...
        let (result, result_error) = divide_with_error(
            RR.get_ring(),
            &sum,
            &sum_error,
            gso.cholesky.at(k, k),
            gso.error_bound.at(k, k),
        );
    ...
    loop {
        ...
        for k in (0..i).rev() {
            let min_entry = RR.sub(
                RR.abs(RR.clone_el(gso.cholesky.at(k, i))),
                // add epsilon here, to avoid `NotEnoughPrecision` error in the case that `mu = +/-
                // 1/2` and `error = 0`
            ...
            let factor = RR
                .get_ring()
                .round_to_integer(BigIntRing::RING, min_mu)
                .ok_or(NotEnoughPrecision)?;
    ...
    V3: AsPointerToSlice<R::Element>,
{
    ...
    if RR.is_lt(&current_norm_squared, &RR.mul(factor, prev_norm_squared)) {
...
///  - The GSO coefficients are always recomputed after a column swap. This is necessary, since
///    continuously adjusting them fundamentally has very bad numerical properties. For this
///    purpose, a variant of the Cholesky decomposition that keeps track of the current error is
///    used.
///  - It may happen that the error after computing the Cholesky decomposition is too large to
///    compute a `(delta, eta)`-LLL reduced basis. In that case, [`NotEnoughPrecision`] is returned.
///    If you are using arbitrary precision floating point numbers, it can make sense to increase
///    the precision and try again.
    ...
    T: TransformTarget<I>,
{
    ...
    while i < gso.quadratic_form.row_count() {
        ...
        size_reduce(
            ...
            &h,
            eta,
            OffsetTransformIndex::new(&mut transform, zero_vector_count),
        )?;
