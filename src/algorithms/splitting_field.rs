...
pub fn extend_number_field<K, Controller>(
    poly_ring: DensePolyRing<K>,
    irred_poly: &El<DensePolyRing<K>>,
    controller: Controller,
) -> (FreeAlgebraHom<K, NumberField>, El<NumberField>)
where
        ...
        LinSolveRing,
{
    ...
    // lhs is the matrix whose columns are the powers of `potential_primitive_element`, w.r.t. the
    // tensor basis
    ...
    *rhs.at_mut(K.rank(), 2) = k.one();
    ...
    if has_sol {
        ...
        return Some((kX, gen_poly, K_generator, L_generator));
    } else {
        return None;
    }
}
...
#[stability::unstable(feature = "enable")]
pub fn extend_galois_field<K>(
    poly_ring: DensePolyRing<K>,
    irred_poly: &El<DensePolyRing<K>>,
) -> (FreeAlgebraHom<K, GaloisField>, El<GaloisField>)
where
    K: RingStore<Type = GaloisFieldBase<DefaultGaloisFieldImpl>>,
{
    ...
    // solving the linear system (taking `O(d^3)` operations in `GF(q)`, or up to `d^(3 -
    // omega)` less with better matrix inversion algirhtms). I decide for solving the system, it
    // feels better.
...
#[stability::unstable(feature = "enable")]
pub fn extend_number_field_promise_is_irreducible<K, Controller>(
    poly_ring: DensePolyRing<K>,
    irred_poly: &El<DensePolyRing<K>>,
    controller: Controller,
) -> (FreeAlgebraHom<K, NumberField>, El<NumberField>)
    ...
    Controller: ComputationController,
{
    ...
    controller.run_computation(
        format_args!(
            "extend_number_field(deg={}, extdeg={})",
        ...
        |controller| {
            ...
            for _ in 0..MAX_PROBABILISTIC_REPETITIONS {
                ...
                let potential_primitive_element = L.add(
                    L.canonical_gen(),
...
pub fn splitting_field<K, F, Controller>(
    ...
    mut create_field: F,
    controller: Controller,
) -> (FreeAlgebraHom<K, K>, Vec<(El<K>, usize)>)
where
    K: RingStore + Clone,
    ...
    Controller: ComputationController,
{
    ...
    controller.run_computation(
        format_args!(
            "splitting_field(base_deg={}, deg={})",
            poly_ring.base_ring().rank(),
            poly_ring.degree(&to_split[0].0).unwrap()
        ),
        |controller| {
            while let Some((next_to_split, multiplicity)) = to_split.pop() {
                ...
                let extend_idx = factorization
                    .iter()
                    .enumerate()
                    .max_by_key(|(_, (f, _))| poly_ring.degree(f).unwrap())
                    .unwrap()
                    .0;
    ...
    Controller: ComputationController,
{
    ...
    let mut current_roots: Vec<(usize, Vec<El<K>>)> = vec![(
        ...
        (0..relevant_indeterminates.len())
            .map(|_| current_embedding.codomain().zero())
            .collect(),
    )];
    ...
    while let Some((set_indeterminates, current_root)) = current_roots.pop() {
        ...
        let ref_current_embedding =
            FreeAlgebraHom::promise_is_well_defined(&base_field, KX.base_ring(), base_field_gen);
        let next_roots_from: El<DensePolyRing<K>> = polys_for_indeterminate[next_indet_idx]
            .iter()
            .map(|f| {
                poly_ring.evaluate(
                    f,
        ...
        if KX.degree(&next_roots_from).unwrap() == 0 {
            ...
            new_root[next_indet_idx] = KX.base_ring().negate(KX.base_ring().div(
                KX.coefficient_at(&next_roots_from, 0),
                KX.coefficient_at(&next_roots_from, 1),
            ));
            ...
            final_roots = final_roots
                .into_iter()
                .map(|root| root.into_iter().map(|x| into_new_field.map(x)).collect())
                .collect();
...
fn test_extend_field() {
    ...
    let [g] = KX.with_wrapped_indeterminate(|X| {
        [X.pow_ref(4) - RingElementWrapper::new(&KX, KX.inclusion().map(K.canonical_gen()))]
    });
    ...
    assert_el_eq!(
        ext_field,
        ext_field.neg_one(),
        ext_field.pow(extension_field_embedding.map(K.canonical_gen()), 2)
    );
    assert_el_eq!(
        ext_field,
        extension_field_embedding.map(K.canonical_gen()),
        ext_field.pow(x, 4)
    );

    crate::homomorphism::generic_tests::test_homomorphism_axioms(
        &extension_field_embedding,
        [(0, 0), (0, 1), (1, 0), (2, 0), (-1, 0), (0, -1), (-1, -1)]
            .into_iter()
            .map(|(a, b)| K.from_canonical_basis([QQ.int_hom().map(a), QQ.int_hom().map(b)])),
    );
...
fn test_variety_from_lex_gb() {
    ...
    for point in &variety {
        assert_el_eq!(
            ...
            L.zero(),
            QQXYZ.evaluate(
                &intersection_unitsphere_twistedcubic[0],
                point.clone_ring_els(L),
                &into_L
            )
        );
        assert_el_eq!(
            ...
            L.zero(),
            QQXYZ.evaluate(
                &intersection_unitsphere_twistedcubic[1],
                point.clone_ring_els(L),
                &into_L
            )
        );
        assert_el_eq!(
            ...
            L.zero(),
            QQXYZ.evaluate(
                &intersection_unitsphere_twistedcubic[2],
                point.clone_ring_els(L),
                &into_L
            )
        );
