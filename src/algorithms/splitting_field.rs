...
pub trait FiniteFieldOperation<OriginalField>
    where OriginalField: ?Sized + PerfectField
{    
    type Output<'a>;

    fn execute<'a, F>(self, field: &'a F) -> Self::Output<'a>
        where F: FiniteRing + Field + LinSolveRing + FactorPolyField + CanIsoFromTo<OriginalField>;
}

///
/// Field such that every finite degree extension field is generated by a primitive element.
/// 
#[stability::unstable(feature = "enable")]
pub trait PerfectField: SelfIso + Field + LinSolveRing + FactorPolyField {

    fn specialize_finite_field<'a, O: FiniteFieldOperation<Self>>(&'a self, op: O) -> Result<O::Output<'a>, ()>;
}

impl<F: SelfIso + FiniteRing + Field + LinSolveRing + FactorPolyField> PerfectField for F {
...
macro_rules! impl_perfect_field_number_fields {
    ($number_field_type:ty, $($lifetimes:lifetime),*) => {

        impl<$($lifetimes,)* I, V, A> PerfectField for $number_field_type
            where I: IntegerRingStore,
                I::Type: IntegerRing,
        ...
        {
            fn specialize_finite_field<'self_lifetime, O: FiniteFieldOperation<Self>>(&'self_lifetime self, _op: O) -> Result<O::Output<'self_lifetime>, ()> {
                Err(())
            }
        }
    };
...
impl_perfect_field_number_fields!{ AsFieldBase<FreeAlgebraImpl<RationalField<I>, V, A>>, }
impl_perfect_field_number_fields!{ AsFieldBase<RingRef<'a, FreeAlgebraImplBase<RationalField<I>, V, A>>>, 'a }
impl_perfect_field_number_fields!{ AsFieldBase<FreeAlgebraImpl<RingRef<'a, RationalFieldBase<I>>, V, A>>, 'a }
impl_perfect_field_number_fields!{ AsFieldBase<RingRef<'a, FreeAlgebraImplBase<RingRef<'b, RationalFieldBase<I>>, V, A>>>, 'a, 'b }
impl_perfect_field_number_fields!{ AsFieldBase<&'a FreeAlgebraImpl<RationalField<I>, V, A>>, 'a }
impl_perfect_field_number_fields!{ AsFieldBase<FreeAlgebraImpl<&'a RationalField<I>, V, A>>, 'a }
impl_perfect_field_number_fields!{ AsFieldBase<&'a FreeAlgebraImpl<&'b RationalField<I>, V, A>>, 'a, 'b }
...
pub fn splitting_field<P>(poly_ring: P, poly: El<P>)
    where P: PolyRingStore,
        ...
        for<'a> AsFieldBase<FreeAlgebraImpl<&'a <P::Type as RingExtension>::BaseRing, Vec<El<<P::Type as RingExtension>::BaseRing>>>>: PerfectField
{
...
fn extend_splitting_field<'a, 'b, R>(poly_ring: &ThisPolyRing<'a, 'b, R>, mut remaining_factors: Vec<(El<ThisPolyRing<'a, 'b, R>>, usize)>)
    where R: RingStore,
        R::Type: PerfectField,
        for<'c> AsFieldBase<FreeAlgebraImpl<&'c R, Vec<El<R>>>>: PerfectField
{
...
struct FiniteFieldCase<'a, 'b, 'c, R>
    where R: RingStore,
        R::Type: PerfectField,
        for<'d> AsFieldBase<FreeAlgebraImpl<&'d R, Vec<El<R>>>>: PerfectField,
        'a: 'b,
        'b: 'c
{
    poly_ring: &'c ThisPolyRing<'a, 'b, R>,
    irred_poly: &'c El<ThisPolyRing<'a, 'b, R>>
}

impl<'a, 'b, 'c, R> FiniteFieldOperation<AsFieldBase<FreeAlgebraImpl<&'a R, Vec<El<R>>>>> for FiniteFieldCase<'a, 'b, 'c, R>
    where R: RingStore,
        R::Type: PerfectField,
        for<'d> AsFieldBase<FreeAlgebraImpl<&'d R, Vec<El<R>>>>: PerfectField,
        'a: 'b,
        'b: 'c
{
    ...
    fn execute<'d, F>(self, field: &'d F) -> Self::Output<'d>
        where F: FiniteRing + Field + LinSolveRing + FactorPolyField + CanIsoFromTo<AsFieldBase<FreeAlgebraImpl<&'a R, Vec<El<R>>>>>
    {
        unimplemented!()
    }
}
        ...
        'b: 'c
{
    ...
    let (result_ring, old_gen, new_gen) = if let Ok(_) = ring.get_ring().specialize_finite_field(FiniteFieldCase { poly_ring, irred_poly: &irred_poly }) {
        unimplemented!()
