        ...
        'b: 'c
{
    ...
    if BigIntRing::RING.is_zero(&characteristic) {
        // all involved fields are Q-vector spaces; it is known that thus `a z + X`
        // generates `extension_field` for all but finitely many rationals `a`, where `z` is the generator of `ring`
        
        let mut rng = oorandom::Rand64::new(1);
        let mut size_bound_a = 1;
        loop {
            let mut lhs = OwnedMatrix::zero(total_rank, total_rank, base_ring);
            let a = StaticRing::<i32>::RING.get_ring().get_uniformly_random_bits(size_bound_a, || rng.rand_u64());
            size_bound_a = min(size_bound_a + 1, i32::BITS as usize - 1);
