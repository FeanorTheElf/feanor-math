...
fn test_primitive_element<R>(L: R, potential_primitive_element: El<R>) -> Option<(
    DensePolyRing<<<BaseRingStore<R> as RingStore>::Type as RingExtension>::BaseRing>, 
    El<DensePolyRing<<<BaseRingStore<R> as RingStore>::Type as RingExtension>::BaseRing>>,
    El<DensePolyRing<<<BaseRingStore<R> as RingStore>::Type as RingExtension>::BaseRing>>,
    El<DensePolyRing<<<BaseRingStore<R> as RingStore>::Type as RingExtension>::BaseRing>>
)>
    where R: RingStore,
        R::Type: FreeAlgebra,
        <BaseRingStore<R> as RingStore>::Type: FreeAlgebra,
        <<BaseRingStore<R> as RingStore>::Type as RingExtension>::BaseRing: Clone,
        <<<BaseRingStore<R> as RingStore>::Type as RingExtension>::BaseRing as RingStore>::Type: LinSolveRing
{
    let K = L.base_ring();
    let k = K.base_ring();
...
pub fn variety_from_lex_gb<K, P, F>(
    ...
    where P: RingStore,
        ...
        BaseRingStore<P>: Borrow<K> + RingStore<Type = K::Type>,
        F: for<'a, 'b> FnMut(DensePolyRing<&'a K, DynConvolution<'b, K::Type>>, El<DensePolyRing<&'a K, DynConvolution<'b, K::Type>>>) -> (FreeAlgebraHom<&'a K, K>, El<K>)
{
