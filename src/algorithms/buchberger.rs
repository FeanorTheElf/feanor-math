    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::LTAnnihilated {
                i,
                annihilator: _,
                annihilate_including: _,
            } => write!(f, "annihilator * F({})", i),
        }
    }
}
    ...
    <BaseRingStore<P> as RingStore>::Ring: PrincipalIdealRing,
{
    ...
    return (
        (
            ring.base_ring().checked_div(&xlcm_c, l_c).unwrap(),
            ring.monomial_div(ring.clone_monomial(&xlcm_m), &l_m).ok().unwrap(),
        ),
        (
            ring.base_ring().checked_div(&xlcm_c, r_c).unwrap(),
            ring.monomial_div(ring.clone_monomial(&xlcm_m), &r_m).ok().unwrap(),
        ),
        (xlcm_c, xlcm_m),
    );
}
    ...
    <BaseRingStore<P> as RingStore>::Ring: PrincipalIdealRing,
{
    (
        ...
        ring.monomial_lcm(ring.clone_monomial(l_m), r_m),
    )
}
        ...
        O: MonomialOrder + Copy,
    {
        match self {
            ...
            SPoly::LTAnnihilated {
                i,
                annihilator: _,
                annihilate_including,
            } => (
                ring.coefficient_at(&basis[*i], annihilate_including).clone(),
                ring.clone_monomial(annihilate_including),
            ),
        }
    }
...
fn filter_spoly<P, O>(
    ring: P,
    new_spoly: &SPoly<PolyCoeff<P>, PolyMonomial<P>>,
    basis: &[El<P>],
    order: O,
) -> Option<usize>
where
    ...
    O: MonomialOrder + Copy,
{
    match new_spoly {
        SPoly::Standard { i, j: k } => {
            ...
            if ring.base_ring().is_unit(&S_c)
                && ring.monomial_deg(&S_m) == ring.monomial_deg(bi_m) + ring.monomial_deg(bk_m)
            {
            ...
            (0..*k)
                .filter_map(|j| {
                    ...
                    // more experiments needed - for some weird reason, replacing "properly divides" with "divides"
                    // (assuming I didn't make a mistake) leads to terrible performance
                    ...
                    if let Ok(quo) = ring.monomial_div(ring.clone_monomial(&S_m), &f_m) {
                        if ring.base_ring().divides(&S_c, &f_c)
                            && (!ring.base_ring().divides(&f_c, &S_c) || ring.monomial_deg(&quo) > 0)
                        {
                            return Some(j);
                        }
                    }
...
fn update_basis<I, P, O, SortFn>(
    ...
    new_polys: I,
    basis: &mut Vec<El<P>>,
    open: &mut Vec<SPoly<PolyCoeff<P>, PolyMonomial<P>>>,
    order: O,
    ...
    for new_poly in new_polys {
        ...
        if !ring.base_ring().is_unit(&ring.LT(new_poly, order).unwrap().0) {
            ...
            for j in 1..terms.len() {
                if !ring
                    .base_ring()
                    .is_zero(&ring.base_ring().mul_ref(terms[j].0, &current_annihilator))
                {
                    let new_annihilator = ring
                        .base_ring()
                        .ideal_intersect(&current_annihilator, &ring.base_ring().annihilator(terms[j].0));
    ...
    I: Iterator<Item = (&'a El<P>, &'b ExpandedMonomial)>,
{
    while let Some((_, reducer, quo_c, quo_m)) = find_reducer(ring, to_reduce, reducers(), order) {
        ...
        let mut scaled_reducer = reducer.clone();
...
#[stability::unstable(feature = "enable")]
#[instrument(skip_all, level = "trace")]
pub fn buchberger_with_strategy<P, O, SortFn, AbortFn>(
    ring: P,
    ...
    AbortFn: FnMut(&[(El<P>, ExpandedMonomial)]) -> bool,
{
    ...
    // this are the basis polynomials we generated; we only append to this, such that the S-polys remain
    // valid
    let input_basis = inter_reduce(
        &ring,
        input_basis.into_iter().map(|f| expand_lm(ring, f, order)).collect(),
        order,
    )
    .into_iter()
    .map(|(f, _)| f)
    ...
    event!(
        Level::TRACE,
        basis_poly_count = basis.len(),
        reducers_count = reducers.len(),
        spoly_count = open.len(),
        filtered_count = filtered_spolys
    );
    ...
    loop {
        ...
        let spolys_to_reduce_index = open
            ...
            .enumerate()
            .rev()
            .filter(|(_, spoly)| ring.monomial_deg(&spoly.cancelled_term(ring, &basis, order).1) > current_deg)
            .next()
            .map(|(i, _)| i + 1)
            .unwrap_or(0);
        ...
        let new_polys: FrozenVec<Box<(<<P as RingStore>::Ring as RingBase>::Element, Vec<usize>)>> = FrozenVec::new();
        ...
        event!(
            Level::TRACE,
            reduced_to_zero = reduced_to_zero.load(Ordering::Relaxed),
            new_basis_polys = considered_spolys - reduced_to_zero.load(Ordering::Relaxed)
        );
        ...
        if new_polys.len() == 0 && open.len() == 0 {
            if changed {
                ...
                return buchberger_with_strategy::<P, O, _, _>(
                    ring,
            ...
            update_basis(
                ring,
                new_polys.iter().map(|x| &**x).map(|(f, _)| f.clone()),
                &mut basis,
                &mut open,
                order,
                &mut filtered_spolys,
...
/// ordering.
///
/// For a variant of this function that allows for more configuration, see
/// [`buchberger_with_strategy()`].
...
fn test_buchberger_small() {
    ...
    let actual = buchberger_with_strategy(
        &ring,
...
fn test_generic_computation() {
    ...
    let gb1 = buchberger_with_strategy(
        &ring,
        basis.iter().map(|f| f.clone()).collect(),
        DegRevLex,
        default_sort_fn(&ring, DegRevLex),
        |_| false,
    );
...
fn test_expensive_gb_1() {
    ...
    let gb = buchberger_with_strategy(
        &ring,
        system.iter().map(|f| f.clone()).collect(),
        DegRevLex,
        default_sort_fn(&ring, DegRevLex),
        |_| false,
    );
    ...
    for f in &part_of_result {
        assert!(ring.is_zero(&multivariate_division(&ring, f.clone(), gb.iter(), DegRevLex)));
    }
