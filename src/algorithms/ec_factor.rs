        ...
        Controller: ComputationController
{
    ...
    log_progress!(controller, "s({})c({})", log2_p, attempts);
    ...
    for _ in 0..attempts {
        ...
        if let Some(d) = Zn.checked_div(&Zn.sub(Zn.add_ref(&x_sqr, &y_sqr), Zn.one()), &Zn.mul(x_sqr, y_sqr)) {
            ...
            if !Zn.integer_ring().is_unit(&possible_factor) && !Zn.integer_ring().eq_el(&possible_factor, Zn.modulus()) {
                return Ok(Some(possible_factor));
            }
        }
...
///  - `s(m)` means that the current parameters are optimized to find a factor having `m` bits
...
///
/// Finds a factor of the given integer, using Lenstra's Elliptic Curve factorization algorithm.
/// 
/// See also [`lenstra_ec_factor_small()`] which explicitly only tries to find small factors.
/// 
#[stability::unstable(feature = "enable")]
...
fn test_ec_factor() {
    let n = 65537 * 65539;
    let actual = lenstra_ec_factor(&Zn::new(n as u64), LogProgress).unwrap_or_else(no_error);
...
fn bench_ec_factor_mersenne_number_58(bencher: &mut Bencher) {
    ...
    bencher.iter(|| {
        let p = lenstra_ec_factor(&ring, LogProgress).unwrap_or_else(no_error);
        assert!(n > 0 && n != 1 && n != p);
        assert!(n % p == 0);
    });
}
...
fn test_ec_factor_large() {
    ...
    let p = StaticRing::<i128>::RING.coerce(&ZZbig, lenstra_ec_factor(&zn_big::Zn::new(&ZZbig, ZZbig.coerce(&StaticRing::<i128>::RING, n)), LogProgress).unwrap());
    let end = Instant::now();
...
fn test_compute_partial_factorization() {
    ...
    let begin = Instant::now();
    let factor = lenstra_ec_factor_small(&Zn, 50, 1, LogProgress).unwrap_or_else(no_error).unwrap();
    let end = Instant::now();
    println!("Done in {} ms", (end - begin).as_millis());
