...
fn lenstra_ec_factor_base<R, F, Controller>(
    Zn: R,
    log2_p: usize,
    mut rng: F,
    controller: Controller,
) -> Result<Option<El<<R::Type as ZnRing>::IntegerRing>>, Controller::Abort>
where
    R: RingStore + Copy + Send + Sync,
    El<R>: Send,
    ...
    Controller: ComputationController,
{
    controller.run_computation(
        format_args!(
            "ec_factor(log2(n)={}, log2(p)={})",
            Zn.integer_ring().abs_log2_ceil(Zn.modulus()).unwrap(),
            log2_p
        ),
        |controller| {
            let ZZ = BigIntRing::RING;
            ...
            let power = ZZ.prod(
                power_factorization
                    .iter()
                    .map(|(p, e)| ZZ.pow(ZZ.coerce(&StaticRing::<i128>::RING, *p), *e)),
            );
            let power_ref = &power;
...
pub fn lenstra_ec_factor_small<R, Controller>(
    Zn: R,
...
where
    R: ZnRingStore + DivisibilityRingStore + Copy + Send + Sync,
    El<R>: Send,
    R::Type: ZnRing + DivisibilityRing,
    Controller: ComputationController,
{
...
#[stability::unstable(feature = "enable")]
pub fn lenstra_ec_factor<R, Controller>(
    Zn: R,
    controller: Controller,
) -> Result<El<<R::Type as ZnRing>::IntegerRing>, Controller::Abort>
where
    R: ZnRingStore + DivisibilityRingStore + Copy + Send + Sync,
    El<R>: Send,
    R::Type: ZnRing + DivisibilityRing,
...
fn test_ec_factor_large() {
    ...
    let p = StaticRing::<i128>::RING.coerce(
        &ZZbig,
        lenstra_ec_factor(
            &zn_big::Zn::new(&ZZbig, ZZbig.coerce(&StaticRing::<i128>::RING, n)),
            controller.clone(),
        )
...
fn test_compute_partial_factorization() {
    ...
    let begin = Instant::now();
    let factor = lenstra_ec_factor_small(&Zn, 50, 1, TEST_LOG_PROGRESS)
        .unwrap_or_else(no_error)
        .unwrap();
    let end = Instant::now();
    println!("Done in {} ms", (end - begin).as_millis());
