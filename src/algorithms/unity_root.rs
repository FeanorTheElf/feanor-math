...
use crate::integer::{BigIntRing, IntegerRing, IntegerRingStore, int_cast};
...
where
    ...
    I::Type: IntegerRing,
{
    let order = factor(ZZ, ring.characteristic(ZZ).unwrap())
        .into_iter()
        .map(|(p, e)| {
        ...
        .fold(
            ZZ.one(),
            |current, next| if ZZ.is_lt(&current, &next) { next } else { current },
        );
    ...
    R::Type: ZnRing,
{
    get_prim_root_of_unity_zn_gen(
        ring,
        &BigIntRing::RING,
        &int_cast(n as i64, BigIntRing::RING, StaticRing::<i64>::RING),
    )
}
...
fn test_get_prim_root_of_unity() {
    ...
    assert!(is_prim_root_of_unity_pow2(
        &ring,
        &get_prim_root_of_unity_pow2(&ring, 4).unwrap(),
        4
    ));
    ...
    assert!(is_prim_root_of_unity_pow2(
        &ring,
        &get_prim_root_of_unity_pow2(&ring, 2).unwrap(),
        2
    ));
    assert!(is_prim_root_of_unity(
        &ring,
        &get_prim_root_of_unity(&ring, 25).unwrap(),
        25
    ));
    ...
    assert!(is_prim_root_of_unity(
        &ring,
        &get_prim_root_of_unity(&ring, 3).unwrap(),
        3
    ));
...
fn test_get_prim_root_of_unity_zn() {
    ...
    assert!(is_prim_root_of_unity(
        &ring,
        &get_prim_root_of_unity_zn(&ring, 2).unwrap(),
        2
    ));
    ...
    assert!(is_prim_root_of_unity(
        &ring,
        &get_prim_root_of_unity_zn(&ring, 4).unwrap(),
        4
    ));
    ...
    assert!(is_prim_root_of_unity(
        &ring,
        &get_prim_root_of_unity_zn(&ring, 5).unwrap(),
        5
    ));
    assert!(is_prim_root_of_unity(
        &ring,
