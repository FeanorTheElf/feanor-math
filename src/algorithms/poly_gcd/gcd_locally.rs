...
fn poly_gcd_monic_coprime_local<P, F>(
    poly_ring: P,
    f: &El<P>,
    g: &El<P>,
    rng: F,
    ...
    F: FnMut() -> u64,
{
    ...
    for idx in 0..reduction.len() {
        ...
        let [d, _] = hensel_lift_factorization(&S_to_F, &SX, &FX, &RX_to_SX.map_ref(poly), &factors[..])
            .try_into()
            .ok()
            .unwrap();
    ...
    F: FnMut() -> u64,
{
    ...
    let lcf = poly_ring.lc(&f).unwrap().clone();
    let lcg = poly_ring.lc(&g).unwrap().clone();
    ...
    <BaseRingStore<P> as RingStore>::Ring: PolyLiftFactorsDomain,
{
    ...
    event!(
        Level::TRACE,
        lhs_deg = poly_ring.degree(f).unwrap(),
        rhs_deg = poly_ring.degree(g).unwrap()
    );
