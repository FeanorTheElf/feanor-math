...
where
    ...
    G: Fn<(B,)>,
{
    extern "rust-call" fn call(&self, args: (A, B)) -> Self::Output { (self.0.call((args.0,)), self.1.call((args.1,))) }
}
    ...
    G: FnOnce<(B,)>,
{
    ...
    extern "rust-call" fn call_once(self, args: ((A, B),)) -> Self::Output { self.call_once(args.0) }
}
...
where
    ...
    G: FnMut<(B,)>,
{
    extern "rust-call" fn call_mut(&mut self, args: ((A, B),)) -> Self::Output { self.call_mut(args.0) }
}
...
where
    ...
    G: Fn<(B,)>,
{
    extern "rust-call" fn call(&self, args: ((A, B),)) -> Self::Output { self.call(args.0) }
}
...
impl<T> FnOnce<(T,)> for IdentityFunction {
    ...
    extern "rust-call" fn call_once(self, args: (T,)) -> Self::Output { args.0 }
}

impl<T> FnMut<(T,)> for IdentityFunction {
    extern "rust-call" fn call_mut(&mut self, args: (T,)) -> Self::Output { args.0 }
}

impl<T> Fn<(T,)> for IdentityFunction {
    extern "rust-call" fn call(&self, args: (T,)) -> Self::Output { args.0 }
}
