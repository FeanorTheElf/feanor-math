...
pub trait PrincipalIdealRing: DivisibilityRing {
    ...
    fn ideal_intersect(&self, lhs: &Self::Element, rhs: &Self::Element) -> Self::Element {
        // we prove that this works by localization. Consider a prime ideal generated by p.
    ...
    fn inv_crt(&self, [a, b]: [&Self::Element; 2], [p, q]: [&Self::Element; 2]) -> Self::Element {
        ...
        self.checked_div(
            &self.fma(b, &self.mul_ref_fst(p, s), self.mul_ref_fst(a, self.mul_ref_fst(q, t))),
            &d,
        )
        .unwrap()
    }
    ...
    Self::Ring: PrincipalIdealRing,
{
    ...
    delegate! { PrincipalIdealRing, fn ideal_intersect(&self, lhs: &El<Self>, rhs: &El<Self>) -> El<Self> }
    ...
    fn lcm(&self, lhs: &El<Self>, rhs: &El<Self>) -> El<Self> { self.ideal_intersect(lhs, rhs) }
}
...
pub mod generic_tests {
        ...
        R::Ring: PrincipalIdealRing,
    {
        ...
        assert!(
            ...
            ring.formatted_el(&expected_unit),
            ring.formatted_el(&ring.zero()),
            ring.formatted_el(&ring.zero())
        );
        ...
        assert!(
            ...
            ring.formatted_el(&expected_zero),
            ...
            for b in &elements {
                for c in &elements {
                    ...
                    assert!(
                        ...
                        ring.formatted_el(a),
                        ring.formatted_el(&g1),
                        ring.formatted_el(&g2),
                        ring.formatted_el(&g),
                        ring.formatted_el(a),
                        ring.formatted_el(&g)
                    );
...
fn test_ideal_intersect() {
    ...
    let ring = Zn::<8>::RING;
    assert_eq!(4, ring.ideal_intersect(&4, &4));
    assert_eq!(4, ring.ideal_intersect(&2, &4));
    assert_eq!(2, ring.ideal_intersect(&2, &2));
}
