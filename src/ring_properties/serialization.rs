...
/// Wrapper of a ring that implements [`serde::DeserializationSeed`] by trying to deserialize an
/// element w.r.t. the wrapped ring.
#[stability::unstable(feature = "enable")]
#[derive(Clone)]
...
fn test_serialize() {
    ...
    let value = ZZbig.add(ZZbig.power_of_two(128), ZZbig.one());
    let json = serde_json::to_string(&SerializeWithRing::new(&value, ZZbig)).unwrap();
