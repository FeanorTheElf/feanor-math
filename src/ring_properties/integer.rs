...
/// change.
#[cfg(feature = "mpir")]
pub type BigIntRing = crate::ring_impls::mpir::MPZ;
/// Type alias for the current default used big integer ring implementation.
///
/// The type this points to may change when features or other compilation parameters
/// change.
...
/// change.
#[cfg(feature = "mpir")]
pub type BigIntRingBase = crate::ring_impls::mpir::MPZBase;
/// Type alias for the current default used big integer ring implementation.
///
/// The type this points to may change when features or other compilation parameters
/// change.
    ...
    Domain + EuclideanRing + OrderedRing + HashableElRing + IntegerPolyLiftFactorsDomain + LiftPolyEvalRing
{
    ...
    ///  - First performing some operation on floating point numbers, and then refining it to
    ///    integers.
    ///
    ...
    ///
    /// For information when to use (or not use) this, see its counterpart
    /// [`IntegerRing::to_float_approx()`].
    ...
    /// `abs(value)`, or `None` if the value is zero.
    ///
    /// # Example
    /// ```rust
    /// # use feanor_math::primitive_int::*;
    ...
    J: ?Sized + IntegerRing,
{
    ...
    default fn mul_assign_map_in_ref(
        &self,
        from: &I,
        lhs: &mut Self::Element,
        rhs: &<I as RingBase>::Element,
...
pub mod generic_impls {
        ...
        I::Ring: IntegerRing,
    {
        ...
        let chunk_base = ring.pow(int_cast(base as i64, &ring, ZZi64), chunk_size);
        let result = it.fold(Ok(ring.zero()), |current, next| {
            current.and_then(|mut current| {
                next.map(|next| {
                    ring.mul_assign_ref(&mut current, &chunk_base);
                    ring.add(current, int_cast(next as i64, &ring, ZZi64))
                })
            })
        });
        if negative {
